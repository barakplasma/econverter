...
class FormatterFunctions(object):
    error_function_body = (
        "def evaluate(self, formatter, kwargs, mi, "
    ...
    def register_builtin(self, func_class):
        if not isinstance(func_class, FormatterFunction):
            raise ValueError(
                "Class %s is not an instance of FormatterFunction"
...
class BuiltinFormatterFunction(FormatterFunction):
    def __init__(self):
        ...
        eval_func = inspect.getmembers(
            self.__class__, lambda x: inspect.ismethod(x) and x.__name__ == "evaluate"
        )
        try:
...
class BuiltinStrcmp(BuiltinFormatterFunction):
    ...
    category = "Relational"
    __doc__ = doc = (
        "strcmp(x, y, lt, eq, gt) -- does a case-insensitive "
...
class BuiltinFirstMatchingCmp(BuiltinFormatterFunction):
    ...
    arg_count = -1
    __doc__ = doc = (
        "first_matching_cmp(val, cmp1, result1, cmp2, r2, ..., "
...
class BuiltinRawField(BuiltinFormatterFunction):
    ...
    category = "Get values from metadata"
    __doc__ = doc = (
        "raw_field(name) -- returns the metadata field named by name "
...
class BuiltinContains(BuiltinFormatterFunction):
    ...
    def evaluate(
        self, formatter, kwargs, mi, locals, val, test, value_if_present, value_if_not
    ):
...
class BuiltinRe(BuiltinFormatterFunction):
    ...
    category = "String manipulation"
    __doc__ = doc = (
        "re(val, pattern, replacement) -- return val after applying "
...
class BuiltinIfempty(BuiltinFormatterFunction):
    ...
    category = "If-then-else"
    __doc__ = doc = (
        "ifempty(val, text if empty) -- return val if val is not empty, "
...
class BuiltinShorten(BuiltinFormatterFunction):
    ...
    def evaluate(
        self, formatter, kwargs, mi, locals, val, leading, center_string, trailing
    ):
        l = max(0, int(leading))
...
class BuiltinFormatsModtimes(BuiltinFormatterFunction):
    ...
    def evaluate(self, formatter, kwargs, mi, locals, fmt):
        ...
        try:
            ...
            return ",".join(
                k.upper() + ":" + format_date(v["mtime"], fmt) for k, v in data
            )
        except:
            return ""
...
class BuiltinFormatsSizes(BuiltinFormatterFunction):
    ...
    def evaluate(self, formatter, kwargs, mi, locals):
        ...
        try:
            return ",".join(
                k.upper() + ":" + str(v["size"]) for k, v in fmt_data.items()
            )
        except:
            return ""
...
class BuiltinFormatsPaths(BuiltinFormatterFunction):
    ...
    def evaluate(self, formatter, kwargs, mi, locals):
        ...
        try:
            return ",".join(
                k.upper() + ":" + str(v["path"]) for k, v in fmt_data.items()
            )
        except:
            return ""
...
class BuiltinHumanReadable(BuiltinFormatterFunction):
    ...
    __doc__ = doc = (
        "human_readable(v) -- return a string "
...
class BuiltinHasCover(BuiltinFormatterFunction):
    ...
    category = "Get values from metadata"
    __doc__ = doc = (
        "has_cover() -- return Yes if the book has a cover, "
...
class BuiltinListEquals(BuiltinFormatterFunction):
    ...
    def evaluate(
        self, formatter, kwargs, mi, locals, list1, sep1, list2, sep2, yes_val, no_val
    ):
...
def load_user_template_functions(library_uuid, funcs, precompiled_user_functions=None):
    ...
    formatter_functions().register_functions(
        library_uuid, list(compiled_funcs.values())
    )
