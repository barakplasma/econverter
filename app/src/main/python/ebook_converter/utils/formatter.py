...
class _Parser(object):
    ...
    def __init__(self, val, prog, funcs, parent):
        ...
        if prog[1] != "":
            self.error("failed to scan program. Invalid input {0}".format(prog[1]))
        self.parent = parent
    ...
    def expr(self):
        if self.token_is_id():
            ...
            return cls.eval_(
                self.parent, self.parent_kwargs, self.parent_book, self.locals, *args
            )
    ...
    def _do_format(self, val, fmt):
        ...
        if typ == "s":
            ...
            try:
                ...
                raise ValueError(
                    "format: type {0} requires an integer value, got {1}".format(
                        typ, val
                    )
                )
            ...
            try:
                ...
                raise ValueError(
                    "format: type {0} requires a decimal (float) value, got {1}".format(
                        typ, val
                    )
                )
            ...
            if p >= 0 and fmt[-1] == ")":
                ...
                if fname in self.funcs:
                    ...
                    if (func.arg_count == 1 and (len(args) != 1 or args[0])) or (
                        ...
                        raise ValueError(
                            "Incorrect number of arguments for function " + fmt[0:p]
                        )
                    if func.arg_count == 1:
                        ...
                        val = func.eval_(
                            self, self.kwargs, self.book, self.locals, val, *args
                        )
                        if self.strip_results:
                            val = val.strip()
                else:
