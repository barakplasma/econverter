        ...
        if isinstance(raw_or_get_table, bytes):
            ...
            if self.sfnt_version not in {
                ...
                raise UnsupportedFont(
                    "Font has unknown sfnt version: %r" % self.sfnt_version
                )
            for (
                table_tag,
                table,
                ...
                self.tables[table_tag] = self.TABLE_MAP.get(table_tag, UnknownTable)(
                    table
                )
        else:
            for table_tag in {
...
def test_roundtrip(ff=None):
    ...
    if len(data) != len(rd):
        raise ValueError(
            "Roundtripping failed, size different (%d vs. %d)" % (len(data), len(rd))
        )
