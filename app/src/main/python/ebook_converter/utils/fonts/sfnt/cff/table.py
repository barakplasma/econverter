...
from ebook_converter.utils.fonts.sfnt.cff.constants import (
...
class CFF(object):
    def __init__(self, raw):
        (self.major_version, self.minor_version, self.header_size, self.offset_size) = (
            unpack_from(b">4B", raw)
        )
        if (self.major_version, self.minor_version) != (1, 0):
            raise UnsupportedFont(
                "The CFF table has unknown version: "
        ...
        if cs_type != 2:
            raise UnsupportedFont(
                "This font has unsupported CharstringType: %s" % cs_type
            )
        ...
        if pd:
            ...
            self.private_dict.decompile(
                self.strings, self.global_subrs, raw[offset : offset + size]
            )
        ...
        self.charset = Charset(
            raw,
            self.top_dict.safe_get("charset"),
            self.strings,
            self.num_glyphs,
            self.is_CID,
        )
...
class Strings(Index):
    def __init__(self, raw, offset):
        super(Strings, self).__init__(
            raw, offset, prepend=[x.encode("ascii") for x in cff_standard_strings]
        )
...
class Charset(list):
    def __init__(self, raw, offset, strings, num_glyphs, is_CID):
        ...
        if self.standard_charset is None:
            ...
            f = {
                0: self.parse_fmt0,
                1: self.parse_fmt1,
                2: partial(self.parse_fmt1, is_two_byte=True),
            }.get(fmt, None)
            if f is None:
                raise UnsupportedFont(
                    "This font uses unsupported charset table format: %d" % fmt
                )
    ...
    def parse_fmt1(self, raw, offset, strings, num_glyphs, is_CID, is_two_byte=False):
        ...
        while count < num_glyphs:
            ...
            count += nleft + 1
            self.extend(
                "cid%05d" % x if is_CID else strings[x]
                for x in range(first, first + nleft + 1)
            )
...
class CFFTable(UnknownTable):
    ...
    def subset(self, character_map, extra_glyphs):
        ...
        charset_map = {
            ...
            for code, glyph_id in character_map.items()
        }
