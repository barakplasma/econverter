    ...
    def read_extra_header(self, data):
        ...
        if self.lookup_order != 0:
            raise UnsupportedFont(
                "This LanguageSystemTable has an unknown"
...
class ScriptTable(ListTable):
    ...
    def read_extra_header(self, data):
        ...
        self[b"default"] = (
            LanguageSystemTable(data.raw, start_pos + default_offset)
            if default_offset
            else None
        )
...
class FeatureTable(IndexTable):
    def read_extra_header(self, data):
        ...
        if False and self.feature_params != 0:
            ...
            raise UnsupportedFont(
                "This FeatureTable has non NULL FeatureParams: 0x%x"
                % self.feature_params
            )
...
def ExtensionSubstitution(raw, offset, subtable_map={}):
    ...
    if subst_format != 1:
        raise UnsupportedFont(
            "ExtensionSubstitution has unknown format: 0x%x" % subst_format
        )
...
class Coverage(object):
    def __init__(self, raw, offset, parent_table_name):
        ...
        if self.format not in {1, 2}:
            raise UnsupportedFont(
                "Unknown Coverage format: 0x%x in %s" % (self.format, parent_table_name)
            )
        if self.format == 1:
...
class UnknownLookupSubTable(object):
    ...
    def __init__(self, raw, offset):
        ...
        if self.format not in self.formats:
            raise UnsupportedFont(
                "Unknown format for Lookup Subtable %s: 0x%x"
