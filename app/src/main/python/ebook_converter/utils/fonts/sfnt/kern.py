...
from ebook_converter.utils.fonts.sfnt import (
    UnknownTable,
    FixedProperty,
    max_power_of_two,
)
    ...
    def restrict_format_0(self, raw, glyph_ids):
        ...
        if offset != len(raw):
            raise UnsupportedFont(
                "This font has extra data at the end of a Format 0 kern subtable"
            )
        ...
        return (
            header
            + pack(b">4H", npairs, search_range, entry_selector, range_shift)
            + entries
        )
