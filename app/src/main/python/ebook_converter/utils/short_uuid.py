...
class ShortUUID(object):
    def __init__(self, alphabet=None):
        ...
        self.alphabet = tuple(
            sorted(str(alphabet or (string.digits + string.ascii_letters)[2:]))
        )
    ...
    def uuid5(self, namespace, name, pad_to_length=None):
        if pad_to_length is None:
            pad_to_length = self.uuid_pad_len
        return num_to_string(
            _uuid.uuid5(namespace, name).int,
            self.alphabet,
            self.alphabet_len,
            pad_to_length,
        )
    ...
    def decode(self, encoded):
        return _uuid.UUID(
            int=string_to_num(encoded, self.alphabet_map, self.alphabet_len)
        )
