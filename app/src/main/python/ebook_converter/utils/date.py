...
try:
    ...
    parse_date_day_first = first_index(raw, ("%d", "%a", "%A")) < first_index(
        raw, ("%m", "%b", "%B")
    )
    del raw, first_index
except Exception:
...
def is_date_undefined(qt_or_dt):
    ...
    return d.year < UNDEFINED_DATE.year or (
        d.year == UNDEFINED_DATE.year
        and d.month == UNDEFINED_DATE.month
        and d.day == UNDEFINED_DATE.day
    )
...
def parse_date(date_string, assume_utc=False, as_utc=True, default=None):
    ...
    if default is None:
        func = datetime.datetime.utcnow if assume_utc else datetime.datetime.now
...
def parse_only_date(raw, assume_utc=True, as_utc=True):
    ...
    return fix_only_date(
        parse_date(raw, default=default, assume_utc=assume_utc, as_utc=as_utc)
    )
...
def qt_to_dt(qdate_or_qdatetime, as_utc=True):
    ...
    try:
        dt = datetime.datetime(_safeyear(o.year()), o.month(), o.day()).replace(
            tzinfo=_local_tz
        )
    except ValueError:
        dt = datetime.datetime(_safeyear(o.year()), o.month(), 1).replace(
            tzinfo=_local_tz
        )
    return dt.astimezone(_utc_tz if as_utc else _local_tz)
