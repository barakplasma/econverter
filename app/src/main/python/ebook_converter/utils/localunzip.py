...
def find_data_descriptor(f):
    ...
    try:
        ...
        raise ValueError(
            "Failed to find data descriptor signature. "
...
def copy_compressed_file(src, size, dest):
    ...
    while read < size:
        ...
        while d.unconsumed_tail:
            ...
            if count > 100:
                raise ValueError(
                    "This ZIP file contains a ZIP bomb in %s"
...
def _extractall(f, path=None, file_info=None):
    ...
    while True:
        ...
        parts = [
            x for x in fname.split("/") if x not in {"", os.path.pardir, os.path.curdir}
        ]
        if not parts:
            continue
        ...
        if path is not None:
            ...
            try:
                ...
                if is_reserved_filename(os.path.basename(dest)):
                    raise ValueError(
                        "This ZIP file contains a file with a reserved filename"
                        " that cannot be processed on Windows: {}".format(
        ...
        with SpooledTemporaryFile(max_size=100 * 1024 * 1024) as temp:
            ...
            for offset, header in self.file_info.values():
                if header.filename in names:
                    ...
                    ztemp.writestr(
                        header.filename, self.read(header.filename, spool_size=0)
                    )
            if add_missing:
