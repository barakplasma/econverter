...
class ZipFile:
    ...
    def __init__(self, file, mode="r", compression=ZIP_DEFLATED, allowZip64=True):
        ...
        if compression == ZIP_STORED:
            ...
            raise RuntimeError(
                "The compression method %s is not supported" % compression
            )
    ...
    def delete(self, name):
        ...
        for i in range(0, len(self.filelist)):
            if self.filelist[i].filename == name:
                ...
                deleted_size = (
                    self.filelist[i].file_offset - self.filelist[i].header_offset
                ) + self.filelist[i].compress_size
                zinfo_size = (
                    struct.calcsize(structCentralDir)
    ...
    def open(self, name, mode="r", pwd=None):
        ...
        if fname != zinfo.orig_filename:
            print(
                (
                    "WARNING: Header (%r) and directory (%r) filenames do not"
        ...
        if is_encrypted:
            ...
            if not pwd:
                raise RuntimeError(
                    ("File %s is encrypted, password required for extraction") % name
                )
    ...
    def _extract_member(self, member, targetpath, pwd):
        ...
        if (
            targetpath[-1:] in (os.path.sep, os.path.altsep)
        ...
        fname = "/".join(
            x for x in fname.split("/") if x not in {"", os.path.curdir, os.path.pardir}
        )
        if not fname:
        ...
        if upperdirs and not os.path.exists(upperdirs):
            try:
                ...
                targetpath = os.path.join(base_target, fms.sanitize_file_name(fname))
                upperdirs = os.path.dirname(targetpath)
                if upperdirs and not os.path.exists(upperdirs):
                    os.makedirs(upperdirs)
    ...
    def write(self, filename, arcname=None, compress_type=None):
        ...
        self.fp.write(
            struct.pack("<LLL", zinfo.CRC, zinfo.compress_size, zinfo.file_size)
        )
