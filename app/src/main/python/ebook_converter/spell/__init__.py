...
def get_codes():
    ...
    if ccodes is None:
        db = json.loads(
            (
                importlib.resources.files("ebook_converter") / "data/iso_3166-1.json"
        ...
        data = {"names": name_map, "codes": frozenset(codes), "three_map": three_map}

        ccodes, ccodemap, country_names = (
            data["codes"],
            data["three_map"],
            data["names"],
        )
