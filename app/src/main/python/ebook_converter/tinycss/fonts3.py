...
STRETCH_IDENTS = frozenset(
    "ultra-condensed extra-condensed condensed semi-condensed semi-expanded expanded extra-expanded ultra-expanded".split()
)
...
class CSSFonts3Parser(CSS21Parser):
    ...
    def parse_font_face_rule(self, rule, previous_rules, errors, context):
        ...
        if rule.head:
            raise ParseError(
                rule,
                "{0} rule is not allowed to have content before the descriptor declaration".format(
                    rule.at_keyword
                ),
            )
