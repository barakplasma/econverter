...
class MediaQuery(object):
    ...
    def __eq__(self, other):
        return (
            self.media_type == getattr(other, "media_type", None)
            and self.negated == getattr(other, "negated", None)
            and self.expressions == getattr(other, "expressions", None)
        )
...
class CSSMedia3Parser(CSS21Parser):
    ...
    def parse_media(self, tokens, errors):
        ...
        for part in split_on_comma(remove_whitespace(tokens)):
                ...
                for i, tok in enumerate(part):
                    ...
                    if not tok.is_container:
                        raise MalformedExpression(
                            tok, "expected a media expression not a %s" % tok.type
                        )
                    if tok.type != "(":
                        raise MalformedExpression(
                            tok,
                    ...
                    if content[0].type != "IDENT":
                        raise MalformedExpression(
                            content[0], "expected a media feature not a %s" % tok.type
                        )
                    media_feature, expr = content[0].value, None
                    if len(content) > 1:
                        if len(content) < 3:
                            raise MalformedExpression(
                                content[1], "malformed media feature definition"
                            )
                        if content[1].type != ":":
                        ...
                        if len(expr) == 1:
                            ...
                            r.type = "RATIO"
                            r._as_css = (
                                expr[0]._as_css + expr[1]._as_css + expr[2]._as_css
                            )
                            expr = r
                        else:
                            raise MalformedExpression(
            ...
            queries.append(
                MediaQuery(
                    media_type or "all", expressions=tuple(expressions), negated=negated
                )
            )
