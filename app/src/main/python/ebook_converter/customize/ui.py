...
def _config():
    ...
    c.add_opt("filetype_mapping", default={}, help="Mapping for filetype plugins")
    c.add_opt("plugin_customization", default={}, help="Local plugin customization")
...
run_plugins_on_preprocess = functools.partial(
    _run_filetype_plugins, occasion="preprocess"
)
run_plugins_on_postprocess = functools.partial(
    _run_filetype_plugins, occasion="postprocess"
)
...
def initialize_plugin(plugin, path_to_zip_file):
    try:
        ...
        raise customize.InvalidPlugin(
            ("Initialization of plugin %s failed with traceback:" % tb) + "\n" + tb
        )
...
def option_parser():
    ...
    parser.add_option(
        ...
        "--add-plugin",
        default=None,
        help="Add a plugin by specifying the path to the ZIP file containing it.",
    )
    parser.add_option(
        "-b",
        "--build-plugin",
    ...
    parser.add_option(
        ...
        "--remove-plugin",
        default=None,
        help="Remove a custom plugin by name. Has no effect on builtin plugins",
    )
    parser.add_option(
        "--customize-plugin",
        default=None,
    ...
    parser.add_option("--enable-plugin", default=None, help="Enable the named plugin")
    parser.add_option("--disable-plugin", default=None, help="Disable the named plugin")
    return parser
