...
class PML2PMLZ(FileTypePlugin):
    ...
    def run(self, pmlfile):
        ...
        img_dir = (
            pml_img if os.path.isdir(pml_img) else i_img if os.path.isdir(i_img) else ""
        )
        if img_dir:
...
class TXT2TXTZ(FileTypePlugin):
    ...
    def _get_image_references(self, txt, base_dir):
        ...
        for m in re.finditer(
            ...
            if (
                ...
                and mimetypes.guess_type(path)[0] in OEB_IMAGES
    ...
    def run(self, path_to_ebook):
        ...
        if images:
            ...
            txtz.write(
                path_to_ebook, os.path.basename(path_to_ebook), zipfile.ZIP_DEFLATED
            )
            # metadata.opf
...
class ComicMetadataReader(MetadataReaderPlugin):
    ...
    def get_metadata(self, stream, ftype):
        ...
        if ftype in {"cbr", "cbz"}:
            ...
            try:
                mi.smart_update(
                    get_comic_metadata(stream, ftype, series_index=series_index)
                )
            except:
                pass
        if ret is not None:
...
class EPUBMetadataReader(MetadataReaderPlugin):
    ...
    def get_metadata(self, stream, ftype):
        from ebook_converter.ebooks.metadata.epub import (
            get_metadata,
            get_quick_metadata,
        )
...
plugins += [
    ...
    if isinstance(x, type) and x.__name__.endswith("MetadataReader")
]
...
class EPUBMetadataWriter(MetadataWriterPlugin):
    ...
    def set_metadata(self, stream, mi, type):
        ...
        set_metadata(
            ...
            add_missing_cover="disable-add-missing-cover" != q,
        )
    ...
    def customization_help(self, gui=False):
        ...
        if gui:
            h = "<i>" + h + "</i>"
        return (
            "Enter {0} below to have the EPUB metadata writer plugin not "
...
plugins += [
    x
    for x in list(locals().values())
    if isinstance(x, type) and x.__name__.endswith("MetadataWriter")
]
...
from ebook_converter.ebooks.conversion.plugins.mobi_output import MOBIOutput, AZW3Output
...
class ActionEmbed(InterfaceActionBase):
    ...
    description = (
        "Embed updated metadata into the actual book files in your calibre library"
    )
...
class ActionTemplateTester(InterfaceActionBase):
    ...
    actual_plugin = (
        "ebook_converter.gui2.actions.show_template_tester:ShowTemplateTesterAction"
    )
...
class ActionShowBookDetails(InterfaceActionBase):
    ...
    actual_plugin = (
        "ebook_converter.gui2.actions.show_book_details:ShowBookDetailsAction"
    )
...
class ActionOpenFolder(InterfaceActionBase):
    ...
    description = "Open the folder that contains the book files in your calibre library"
...
class ActionConnectShare(InterfaceActionBase):
    ...
    description = (
        "Send books via email or the web. Also connect to"
...
class ActionEditCollections(InterfaceActionBase):
    ...
    actual_plugin = (
        "ebook_converter.gui2.actions.edit_collections:EditCollectionsAction"
    )
    description = "Edit the collections in which books are placed on your device"
...
class ActionUnpackBook(InterfaceActionBase):
    ...
    description = "Make small changes to EPUB or HTMLZ files in your calibre library"
...
class ActionNextMatch(InterfaceActionBase):
    ...
    description = (
        "Find the next or previous match when searching in "
...
class ActionPluginUpdater(InterfaceActionBase):
    name = "Plugin Updater"
    author = "Grant Drake"
    description = "Get new ebook_converter plugins or update your existing ones"
