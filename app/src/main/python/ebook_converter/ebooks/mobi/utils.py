...
def test_decint(num):
    for d in (True, False):
        ...
        if (num, sz) != decint(raw, forward=d):
            raise ValueError(
                "Failed for num %d, forward=%r: %r != %r"
...
def detect_periodical(toc, log):
    ...
    for node in toc.iterdescendants():
        ...
        if node.depth() == 2 and node.klass != "section":
            log.debug(
                'Not a periodical: Second deepest node does not have class="section"'
            )
            return False
        if node.depth() == 3 and node.klass != "periodical":
            log.debug(
                'Not a periodical: Third deepest node does not have class="periodical"'
            )
            return False
        if node.depth() > 3:
...
def read_font_record(data, extent=1040):
    ...
    ans["ext"] = (
        "ttf"
        if sig in {b"\0\1\0\0", b"true", b"ttcf"}
        else "otf"
        if sig == b"OTTO"
        else "dat"
    )
...
def write_font_record(data, obfuscate=True, compress=True):
    ...
    header = b"FONT" + struct.pack(
        b">5L", usize, flags, data_start, len(xor_key), key_start
    )
...
class CNCX(object):  # {{{
    ...
    def __init__(self, strings=()):
        ...
        RECORD_LIMIT = (
            0x10000 - 1024
        )  # kindlegen appears to use 1024, PDB limit is 0x10000
...
def is_guide_ref_start(ref):
    return ref.title.lower() == "start" or (
        ref.type and ref.type.lower() in {"start", "other.start", "text"}
    )
