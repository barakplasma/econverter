...
ID_RE = re.compile(rb"""<[^>]+\s(?:id|ID)\s*=\s*['"]([^'"]+)['"]""")
NAME_RE = re.compile(rb"""<\s*a\s*\s(?:name|NAME)\s*=\s*['"]([^'"]+)['"]""")
AID_RE = re.compile(rb"""<[^>]+\s(?:aid|AID)\s*=\s*['"]([^'"]+)['"]""")
Part = collections.namedtuple("Part", "num type filename start end aid")
Elem = collections.namedtuple(
    "Elem", "insert_pos toc_text file_number sequence_number start_pos length"
)
...
def get_first_resource_index(
    first_image_index, num_of_text_records, first_text_record_number
):
...
class Mobi8Reader(object):
    ...
    def __call__(self):
        ...
        if self.mobi6_reader.kf8_type == "joint":
            ...
            self.resource_offsets = [
                (_gfri(bh.first_image_index, bh.mobi6_records, 1), offset - 2),
                (
                    _gfri(bh.kf8_first_image_index, bh.records, offset),
            ...
            self.resource_offsets = [
                (
                    _gfri(bh.first_image_index, bh.records, offset),
                    len(self.mobi6_reader.sections),
                )
            ]
    ...
    def read_indices(self):
        ...
        if self.header.skelidx != NULL_INDEX:
            table = read_index(
                self.kf8_sections, self.header.skelidx, self.header.codec
            )[0]
            File = collections.namedtuple(
                "File", "file_number name divtbl_count start_position length"
            )

            for i, text in enumerate(table):
                tag_map = table[text]
                self.files.append(
                    File(i, text, tag_map[1][0], tag_map[6][0], tag_map[6][1])
                )

        self.elems = []
        if self.header.dividx != NULL_INDEX:
            table, cncx = read_index(
                self.kf8_sections, self.header.dividx, self.header.codec
            )
            for i, text in enumerate(table):
                tag_map = table[text]
        ...
        if self.header.othidx != NULL_INDEX:
            table, cncx = read_index(
                self.kf8_sections, self.header.othidx, self.header.codec
            )
    ...
    def build_parts(self):
        ...
        for skelnum, skelname, divcnt, skelpos, skellen in self.files:
            ...
            for i in range(divcnt):
                insertpos, idtext, filenum, seqnum, startpos, length = self.elems[
                    divptr
                ]
                if i == 0:
                    aidtext = idtext[12:-2]
                ...
                if tail.find(b">") < tail.find(b"<") or head.rfind(b">") < head.rfind(
                    ...
                    if not inspos_warned:
                        self.log.warning(
                            "The div table for %s has incorrect "
                            "insert positions. Calculating "
                            "manually.",
                            skelname,
                        )
                        inspos_warned = True
                    bp, ep = locate_beg_end_of_tag(
                        skeleton,
                        aidtext
                        if isinstance(aidtext, bytes)
                        else aidtext.encode("utf-8"),
                    )
                    if bp != ep:
            ...
            self.partinfo.append(
                Part(skelnum, "text", filename, skelpos, baseptr, aidtext)
            )
        ...
        image_tag_pattern = re.compile(rb"""(<(?:svg:)?image[^>]*>)""", re.IGNORECASE)
        for j in range(1, len(self.flows)):
            ...
            if m is not None:
                ...
                if m2 is not None:
                    ...
                    flowpart = re.sub(rb"(</?)svg:", r"\1", flowpart[start:])
                else:
                    format = "file"
                    dir = "images"
    ...
    def create_ncx(self):
        index_entries = read_ncx(
            self.kf8_sections, self.header.ncxidx, self.header.codec
        )
        remove = []
        ...
        for entry in index_entries:
            ...
            if pos_fid is None:
                ...
                try:
                    ...
                    self.log.warning(
                        "Invalid entry in NCX (title: %s), ignoring", entry["text"]
                    )
                    remove.append(entry)
                    continue
            ...
            if isinstance(idtag, bytes):
                entry["idtag"] = idtag.decode(self.header.codec or "utf-8", "strict")
            else:
                entry["idtag"] = idtag
        ...
        for start, end in self.resource_offsets:
            for i, sec in enumerate(sections[start:end]):
                ...
                if typ in {
                    ...
                    if font["err"]:
                        self.log.warning(
                            "Reading font record %d failed: %s", fname_idx, font["err"]
                        )
                        if font["headers"]:
                            self.log.debug("Font record headers: %s", font["headers"])
                    with open(href.replace("/", os.sep), "wb") as f:
                        f.write(
                            font["font_data"] if font["font_data"] else font["raw_data"]
                        )
                    if font["encrypted"]:
                    ...
                    if not (
                        len(data) == len(PLACEHOLDER_GIF) and data == PLACEHOLDER_GIF
                    ):
    ...
    def read_inline_toc(self, href, frag):
        ...
        for elem in root.iterdescendants(etree.Element):
            if (
                reached
                and elem.tag == base.tag("xhtml", "a")
                and elem.get("href", False)
            ):
                href = elem.get("href")
                href, frag = urllib.parse.urldefrag(href)
                href = base_href + "/" + href
