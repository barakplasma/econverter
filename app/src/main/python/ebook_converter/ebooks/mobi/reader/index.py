...
from ebook_converter.ebooks.mobi.utils import decint, count_set_bits, decode_string
...
INDEX_HEADER_FIELDS = (
    ...
    + tuple("unknown%d" % i for i in range(27))
...
def parse_indx_header(data):
    ...
    if ordt2 > 0 and data[ordt2 : ordt2 + 4] == b"ORDT":
        ans["ordt2_raw"] = raw = bytearray(
            data[ordt2 + 4 : ordt2 + 4 + 2 * ans["oentries"]]
        )
        if ans["code"] == 65002:
                ...
                if length > 0:
                    try:
                        self.records[pos + record_offset] = raw[
                            pos + consumed : pos + consumed + length
                        ].decode(codec)
                    except:
                        byts = raw[pos:]
                        r = format_bytes(byts)
                        print(
                            "CNCX entry at offset %d has unknown format %s"
                            % (pos + record_offset, r)
                        )
...
def parse_index_record(
    table, data, control_byte_count, tags, codec, ordt_map, strict=False
):
...
def read_index(sections, idx, codec):
    ...
    for i in range(idx + 1, idx + 1 + indx_count):
        ...
        data = sections[i][0]
        parse_index_record(
            table, data, control_byte_count, tags, codec, indx_header["ordt_map"]
        )
    return table, cncx
