...
def build_exth(
    ...
    if thumbnail_offset is not None:
        ...
        thumbnail_uri_str = (
            "kindle:embed:%s" % to_base(thumbnail_offset, base=32, min_num_digits=4)
        ).encode("utf-8")
        exth.write(
            pack(b">II", EXTH_CODES["kf8_thumbnail_uri"], len(thumbnail_uri_str) + 8)
        )
    ...
    if kf8_unknown_count is not None:
        exth.write(
            pack(b">III", EXTH_CODES["kf8_unknown_count"], 12, kf8_unknown_count)
        )
        nrecs += 1
