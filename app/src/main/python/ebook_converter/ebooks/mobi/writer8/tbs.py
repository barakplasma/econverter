...
from ebook_converter.ebooks.mobi.utils import encode_tbs, encode_trailing_data

Entry = namedtuple(
    "IndexEntry",
    "index start length depth parent "
...
def fill_entry(entry, start_offset, text_record_length):
    ...
    return Entry(
        *(entry[:-4] + (action, start_offset, length_offset, text_record_length))
    )
    ...
    if children:
        ...
        for entry in list(entries):
            if (
                ...
                and entry.parent == parent.parent
    ...
    for rec_length in text_record_lengths:
        ...
        for entry in entries:
            ...
            local_entries.append(
                fill_entry(entry, entry.start - record_start, rec_length)
            )
