...
class MobiMLizer(object):
    ...
    def mobimlize_content(self, tag, text, bstate, istates):
        ...
        if tag in SPECIAL_TAGS and not text:
            ...
            if tag in NESTABLE_TAGS and not istate.rendered:
                ...
                para = wrapper = etree.SubElement(
                    parent, base.tag("xhtml", "blockquote")
                )
                para = wrapper
        ...
        if not pstate or istate != pstate:
            ...
            if fsize != 3:
                inline = etree.SubElement(
                    inline, base.tag("xhtml", "font"), size=str(fsize)
                )
        ...
        content = (
            self.preize_text(text, pre_wrap=istate.pre_wrap)
    ...
    def mobimlize_elem(self, elem, stylizer, bstate, istates, ignore_valign=False):
        ...
        ff = (
            style["font-family"].lower()
            if hasattr(style["font-family"], "lower")
            else ""
        )
        ...
        if tag == "img" and "src" in elem.attrib:
            ...
            if "width" not in istate.attrib or "height" not in istate.attrib:
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            "width" not in istate.attrib
                            and "height" not in istate.attrib
                        ):
            ...
            tag == "hr"
            and asfloat(style["width"]) > 0
            and style._get("width") not in {"100%", "auto"}
        ):
        ...
        not_baseline = valign in (
            ...
            "top",
            "bottom",
        ) or (isinstance(valign, numbers.Number) and abs(valign) != 0)
        ...
        if (
            ...
            or (
                ...
                # be added.
                istate.ids and tag in ("a", "span", "i", "b", "u") and len(elem) == 0
            )
        ):
        ...
        if isblock:
            ...
            if para is not None and para.text == "\xa0" and len(para) < 1:
                if style.height > 2:
                    para.getparent().replace(
                        para, etree.Element(base.tag("xhtml", "br"))
                    )
                else:
