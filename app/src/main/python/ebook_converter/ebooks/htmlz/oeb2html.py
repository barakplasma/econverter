...
class OEB2HTMLNoCSSizer(OEB2HTML):
    ...
    def dump_text(self, elem, stylizer, page):
        ...
        if not isinstance(elem.tag, (str, bytes)) or parse_utils.namespace(
            elem.tag
        ) not in (const.XHTML_NS, const.SVG_NS):
            p = elem.getparent()
            if (
                p is not None
                and isinstance(p.tag, (str, bytes))
                and parse_utils.namespace(p.tag) in (const.XHTML_NS, const.SVG_NS)
                and elem.tail
            ):
                return [elem.tail]
            return [""]
        ...
        for key, value in attribs.items():
            at += ' %s="%s"' % (
                key,
                entities.prepare_string_for_xml(value, attribute=True),
            )
...
class OEB2HTMLInlineCSSizer(OEB2HTML):
    ...
    def dump_text(self, elem, stylizer, page):
        ...
        for k, v in attribs.items():
            at += ' %s="%s"' % (k, entities.prepare_string_for_xml(v, attribute=True))
...
class OEB2HTMLClassCSSizer(OEB2HTML):
    ...
    def mlize_spine(self, oeb_book):
        ...
        title = "<title>%s</title>" % entities.prepare_string_for_xml(self.book_title)
        output = (
            [
                '<html><head><meta http-equiv="Content-Type" '
                'content="text/html;charset=utf-8" />'
