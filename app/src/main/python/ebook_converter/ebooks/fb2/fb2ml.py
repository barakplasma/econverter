...
class FB2MLizer(object):
    ...
    def fb2mlize_spine(self):
        ...
        if self.opts.pretty_print:
            output = etree.tostring(
                etree.fromstring(output), encoding="unicode", pretty_print=True
            )
    ...
    def clean_text(self, text):
        ...
        text = re.sub(
            r"(?mu)<(strong|emphasis|strikethrough|sub|sup)>"
            r"(\s*)</\1>",
            r"\2",
            text,
        )
    ...
    def fb2_header(self):
        ...
        if self.oeb_book.metadata.series:
            ...
            seq = entities.prepare_string_for_xml(str(self.oeb_book.metadata.series[0]))
            metadata["sequence"] = '<sequence name="%s" number="%s"/>' % (seq, index)

        year = publisher = isbn = ""
        identifiers = self.oeb_book.metadata["identifier"]
        for x in identifiers:
            if x.get(base.tag("opf", "scheme"), None).lower() == "uuid" or str(
                x
            ).startswith("urn:uuid:"):
        ...
        try:
            ...
            year = "<year>%s</year>" % entities.prepare_string_for_xml(
        ...
        try:
            ...
            publisher = "<publisher>%s</publisher>" % entities.prepare_string_for_xml(
                publisher.value
            )

        for x in identifiers:
            if x.get(base.tag("opf", "scheme"), None).lower() == "isbn":
                isbn = "<isbn>%s</isbn>" % entities.prepare_string_for_xml(x.value)
        ...
        try:
            ...
            annot = entities.prepare_string_for_xml(html2text(comments.value).strip())
    ...
    def get_cover(self):
        ...
        if (
            self.oeb_book.metadata.cover
            and str(self.oeb_book.metadata.cover[0]) in self.oeb_book.manifest.ids
        ):
        ...
        if cover_href:
            ...
            if (
                cover_href in self.oeb_book.manifest.hrefs
                and cover_href not in self.image_hrefs
            ):
                self.image_hrefs[cover_href] = "img_%s" % len(self.image_hrefs)
            return (
                '<coverpage><image l:href="#%s"/></coverpage>'
    ...
    def get_text(self):
        ...
        for item in self.oeb_book.spine:
            ...
            stylizer = Stylizer(
                item.data, item.href, self.oeb_book, self.opts, self.opts.output_profile
            )
            ...
            text += self.dump_text(
                item.data.find(base.tag("xhtml", "body")), stylizer, item
            )
        ...
        for item in self.oeb_book.manifest:
            ...
            if item.media_type in base.OEB_RASTER_IMAGES:
                try:
                    ...
                    step = 72
                    data = "\n".join(
                        raw_data[i : i + step] for i in range(0, len(raw_data), step)
                    )
                    images.append(
                    ...
                    self.log.error(
                        "Error: Could not include file %s because %s.", item.href, e
                    )
        return "\n".join(images)
    ...
    def dump_text(self, elem_tree, stylizer, page, tag_stack=[]):
        ...
        if (
            not isinstance(elem_tree.tag, (str, bytes))
            or parse_utils.namespace(elem_tree.tag) != const.XHTML_NS
        ):
            p = elem.getparent()
            if (
                p is not None
                and isinstance(p.tag, (str, bytes))
        ...
        if (
            style["display"] in ("none", "oeb-page-head", "oeb-page-foot")
        ...
        if tag == "a" and elem_tree.attrib.get("href", None):
            ...
            if urllib.parse.urlparse(elem_tree.attrib["href"]).netloc:
                ...
                tags += p_tag
                fb2_out.append(
                    '<a l:href="%s">' % base.urlnormalize(elem_tree.attrib["href"])
                )
                tags.append("a")
