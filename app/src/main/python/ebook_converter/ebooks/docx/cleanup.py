...
def cleanup_markup(log, root, styles, dest_dir, detect_cover, XPath):
    ...
    pancestor = XPath(
        "|".join(
            "ancestor::%s[1]" % x for x in ("p", "h1", "h2", "h3", "h4", "h5", "h6")
        )
    )
    ...
    for parent in root.xpath(
        "//*[(%s)]" % " or ".join('name()="%s"' % t for t in parents)
    ):
    ...
    # element
    for parent in root.xpath(
        "//*[(%s) and count(span)=1]" % " or ".join('name()="%s"' % t for t in parents)
    ):
        if (
            len(parent) == 1
            and not parent.text
            and not parent[0].tail
            and not parent[0].get("id", None)
        ):
