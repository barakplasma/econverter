...
parse_hyperlink = parser(
    "hyperlink", "l:anchor m:image-map n:target o:title t:target", "url"
)

parse_xe = parser(
    "xe",
    "b:bold i:italic f:entry-type r:page-range-bookmark t:page-number-text y:yomi",
    "text",
)
...
class Fields(object):
        ...
        for field in self.fields:
            ...
            if field.instructions:
                ...
                if func is not None:
                    ...
                    log.warning(
                        "Encountered unknown field: %s, ignoring it.", field.name
                    )
    ...
    def parse_ref(self, field, parse_func, log):
        ...
        if dest is not None and "hyperlink" in ref:
            ...
            log.warning(
                "Unsupported reference field (%s), ignoring: %r", field.name, ref
            )
    ...
    def parse_index(self, field, parse_func, log):
        ...
        hyperlinks, blocks = process_index(
            field, idx, self.xe_fields, log, self.namespace.XPath, self.namespace.expand
        )
        if not blocks:
            return
