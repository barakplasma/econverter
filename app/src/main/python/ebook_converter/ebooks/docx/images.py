...
class Images(object):
    ...
    def read_image_data(self, fname, base=None):
        ...
        if ext == "emf":
            ...
            self.log.info(
                "Found an EMF image: %s, trying to extract embedded raster image", fname
            )
    ...
    def generate_filename(
        self, rid, base=None, rid_map=None, max_width=None, max_height=None
    ):
            ...
            for a in XPath("descendant::a:blip[@r:embed or @r:link]")(pic):
                ...
                if rid and rid in self.rid_map:
                    try:
                        ...
                        self.log.warning(
                            "Linked image: %s not found, ignoring", err.fname
                        )
                        continue
            ...
            for pic in XPath("descendant::pic:pic")(inline):
                ...
                if ans is not None:
                    if style:
                        ans.set(
                            "style",
                            "; ".join("%s: %s" % (k, v) for k, v in style.items()),
                        )
                    yield ans
        ...
        for imagedata in XPath("descendant::v:imagedata[@r:id]")(pict):
            ...
            if rid in self.rid_map:
                try:
                    ...
                    self.log.warning("Linked image: %s not found, ignoring", err.fname)
                    continue
