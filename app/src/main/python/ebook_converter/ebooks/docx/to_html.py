...
class Convert(object):
    ...
    def __call__(self):
        ...
        self.numbering.apply_markup(
            numbered, self.body, self.styles, self.object_map, self.images
        )
        self.apply_frames()
        ...
        self.cover_image = cleanup_markup(
            self.log,
            self.html,
            self.styles,
            self.dest_dir,
    ...
    def read_styles(self, relationships_by_type):
        ...
        footnotes(
            etree.fromstring(foraw) if foraw else None,
            forel,
            etree.fromstring(enraw) if enraw else None,
            enrel,
        )
        ...
        if fname is not None:
            ...
            try:
                ...
                fonts(
                    etree.fromstring(raw), embed_relationships, self.docx, self.dest_dir
                )
    ...
    def write(self, doc):
        ...
        css = self.styles.generate_css(
            self.dest_dir, self.docx, self.notes_nopb, self.nosupsub
        )
        if css:
        ...
        def process_guide(E, guide):
            if self.toc_anchor is not None:
                guide.append(
                    E.reference(
                        href="index.html#" + self.toc_anchor,
        ...
        with (
            open(os.path.join(self.dest_dir, "metadata.opf"), "wb") as of,
            open(toc_file, "wb") as ncx,
        ):
    ...
    def read_block_anchors(self, doc):
        doc_anchors = frozenset(
            self.namespace.XPath("./w:body/w:bookmarkStart[@w:name]")(doc)
        )
        if doc_anchors:
            current_bm = set()
    ...
    def convert_p(self, p):
        ...
        for x in self.namespace.descendants(
            p, "w:r", "w:bookmarkStart", "w:hyperlink", "w:instrText"
        ):
            ...
            if x.tag.endswith("}r"):
                ...
                x.tag.endswith("}instrText")
                and x.text
                and x.text.strip().startswith("TOC ")
            ):
                old_anchor = current_anchor
        ...
        if len(dest) > 0 and not dest[-1].tail:
            if dest[-1].tag == "br":
                ...
                len(dest[-1]) > 0 and dest[-1][-1].tag == "br" and not dest[-1][-1].tail
            ):
                dest[-1][-1].tail = NBSP
        ...
        for hyperlink, runs in self.fields.hyperlink_fields:
            ...
            if url is None:
                ...
                if anchor in self.anchor_map:
                    span.set("href", "#" + self.anchor_map[anchor])
                    continue
                self.log.warning("Hyperlink field with unknown anchor: %s", anchor)
            else:
            ...
            if self.namespace.is_tag(child, "w:t"):
                ...
                if typ in {"column", "page"}:
                    ...
                    if clear in {"all", "left", "right"}:
                        br = BR(
                            style="clear:%s" % ("both" if clear == "all" else clear)
                        )
                    else:
                        br = BR()
                text.add_elem(br)
                ans.append(text.elem)
            elif self.namespace.is_tag(child, "w:drawing") or self.namespace.is_tag(
                child, "w:pict"
            ):
                for img in self.images.to_html(
                    child, self.current_page, self.docx, self.dest_dir
                ):
                    text.add_elem(img)
                    ans.append(text.elem)
            elif self.namespace.is_tag(
                child, "w:footnoteReference"
            ) or self.namespace.is_tag(child, "w:endnoteReference"):
                anchor, name = self.footnotes.get_ref(child)
                if anchor and name:
                    _l = A(name, id="back_%s" % anchor, href="#" + anchor, title=name)
...
if __name__ == "__main__":
    ...
    os.mkdir(dest_dir)
    Convert(sys.argv[-1], dest_dir=dest_dir, log=logging.default_log)()
