...
class Level(object):
    ...
    def css(self, images, pic_map, rid_map):
        ...
        if self.pic_id:
            ...
            if rid:
                try:
                    fname = images.generate_filename(
                        rid, rid_map=rid_map, max_width=20, max_height=20
                    )
                except Exception:
                    fname = None
                else:
...
class NumberingDefinition(object):
    ...
    def copy(self):
        ans = NumberingDefinition(
            self.namespace, an_id=self.abstract_numbering_definition_id
        )
