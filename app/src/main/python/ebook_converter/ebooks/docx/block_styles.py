...
def read_indent(parent, dest, XPath, get):
    ...
    for indent in XPath("./w:ind")(parent):
        ...
        pl = (
            simple_float(lc, 0.01)
            if lc is not None
            else simple_float(l, 0.05)
            if l is not None
            else None
        )
        if pl is not None:
        ...
        pr = (
            simple_float(rc, 0.01)
            if rc is not None
            else simple_float(r, 0.05)
            if r is not None
            else None
        )
        if pr is not None:
        ...
        if ti is not None:
            text_indent = "%.3g%s" % (
                ti,
                "em" if hc is not None or (h is None and flc is not None) else "pt",
            )
...
def read_spacing(parent, dest, XPath, get):
    ...
    for s in XPath("./w:spacing")(parent):
        a, al, aa = (
            get(s, "w:after"),
            get(s, "w:afterLines"),
            get(s, "w:afterAutospacing"),
        )
        pb = (
            None
            if aa in {"on", "1", "true"}
            else simple_float(al, 0.02)
            if al is not None
            else simple_float(a, 0.05)
            if a is not None
            else None
        )
        if pb is not None:
        ...
        b, bl, bb = (
            get(s, "w:before"),
            get(s, "w:beforeLines"),
            get(s, "w:beforeAutospacing"),
        )
        pt = (
            None
            if bb in {"on", "1", "true"}
            else simple_float(bl, 0.02)
            if bl is not None
            else simple_float(b, 0.05)
            if b is not None
            else None
        )
        if pt is not None:
        ...
        if l is not None:
            lh = (
                ...
                else simple_float(l, 1 / 240.0)
            )
            if lh is not None:
                line_height = "%.3g%s" % (
                    lh,
                    "pt" if lr in {"exact", "atLeast"} else "",
                )
...
class ParagraphStyle(object):
    ...
    def __init__(self, namespace, pPr=None):
        ...
        if pPr is None:
            ...
            for p in (
                ...
                setattr(
                    self, p, binary_property(pPr, p, namespace.XPath, namespace.get)
                )
            ...
            self.font_family = self.font_size = self.color = self.cs_font_size = (
                self.cs_font_family
            ) = inherit
    ...
    def has_visible_border(self):
        for edge in border_edges[:-1]:
            bw, bs = (
                getattr(self, "border_%s_width" % edge),
                getattr(self, "border_%s_style" % edge),
            )
