...
WORD_TYPES = {
    "/word/footnotes.xml": "application/vnd.openxmlformats-"
...
def create_skeleton(opts, namespaces=None):
    ...
    dn = {
        ...
        if k in {"w", "r", "m", "ve", "o", "wp", "w10", "wne", "a", "pic"}
    }
    ...
    styles = E.styles(
        E.docDefaults(
            ...
            E.pPrDefault(
                E.pPr(
                    E.spacing(
                        **{w("after"): "0", w("line"): "276", w("lineRule"): "auto"}
                    )
                )
            ),
        )
...
class DOCX(object):
    def __init__(self, opts, log):
        ...
        self.font_table = etree.Element(
            "{%s}fonts" % namespaces["w"], nsmap={k: namespaces[k] for k in "wr"}
        )
        self.numbering = etree.Element(
            "{%s}numbering" % namespaces["w"], nsmap={k: namespaces[k] for k in "wr"}
        )
        E = ElementMaker(namespace=namespaces["pr"], nsmap={None: namespaces["pr"]})
    ...
    def contenttypes(self):
        ...
        for ext in added:
            types.append(
                E.Default(
                    Extension=ext, ContentType=mimetypes.guess_type("a." + ext)[0]
                )
            )
        for ext, mt in {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    def write(self, path_or_stream, mi, create_empty_document=False):
        ...
        with ZipFile(path_or_stream, "w") as zf:
            ...
            zf.writestr(
                "word/_rels/document.xml.rels", self.document_relationships.serialize()
            )
