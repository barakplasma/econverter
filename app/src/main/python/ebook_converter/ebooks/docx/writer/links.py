...
class TOCItem(object):
    ...
    def serialize(self, body, makeelement):
        ...
        makeelement(
            ...
            w_firstLineChars="0",
            w_firstLine="0",
            w_leftChars=str(200 * self.level),
        )
        if self.is_first:
    ...
    def serialize_hyperlink(self, parent, link):
        ...
        if not purl.scheme:
            ...
            if href in self.document_hrefs:
                ...
                self.log.warning(
                    "Ignoring internal hyperlink with href (%s) "
                    "pointing to unknown destination",
                    url,
                )
        if purl.scheme in {"http", "https", "ftp"}:
            if url not in self.external_links:
                self.external_links[url] = self.document_relationships.add_relationship(
                    url, self.namespace.names["LINKS"], target_mode="External"
                )
