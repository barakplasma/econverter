...
class ImagesManager(object):
    def __init__(self, oeb, document_relationships, opts):
        ...
        self.page_width, self.page_height = (
            opts.output_profile.width_pts,
            opts.output_profile.height_pts,
        )
        self.images = {}
    ...
    def create_image_markup(self, html_img, stylizer, href, as_block=False):
        ...
        if as_block:
            ...
            if (
                ...
                and not (html_img.tail or "").strip()
            ):
        ...
        scaled, width, height = uimg.fit_image(
        ...
        if floating is None:
            ...
            makeelement(
                makeelement(parent, "wp:positionH", relativeFrom="margin"), "wp:align"
            ).text = floating
            makeelement(
                makeelement(parent, "wp:positionV", relativeFrom="line"), "wp:align"
            ).text = "top"
        ...
        if fake_margins:
            ...
            makeelement(
                parent,
                "wp:effectExtent",
                **{k[-1].lower(): v for k, v in get_image_margins(style).items()},
            )
        else:
        ...
        self.create_docx_image_markup(
            parent, name, html_img.get("alt") or name, img.rid, width, height
        )
        return ans
    ...
    def create_docx_image_markup(self, parent, name, alt, img_rid, width, height):
        ...
        (
            makeelement(xfrm, "a:off", x="0", y="0"),
            makeelement(xfrm, "a:ext", cx=str(width), cy=str(height)),
        )
    ...
    def serialize(self, images_map):
        for img in self.images.values():
            images_map["word/" + img.fname] = functools.partial(self.get_data, img.item)
    ...
    def create_cover_markup(self, img, preserve_aspect_ratio, width, height):
        ...
        ans = makeelement(root, "w:drawing", append=False)
        parent = makeelement(
            ans, "wp:anchor", **{"dist" + edge: "0" for edge in "LRTB"}
        )
        (
            parent.set("simplePos", "0"),
            parent.set("relativeHeight", "1"),
        ...
        makeelement(parent, "wp:simplePos", x="0", y="0")
        makeelement(
            makeelement(parent, "wp:positionH", relativeFrom="page"), "wp:align"
        ).text = "center"
        makeelement(
            makeelement(parent, "wp:positionV", relativeFrom="page"), "wp:align"
        ).text = "center"
        width, height = map(pt_to_emu, (width, height))
        makeelement(parent, "wp:extent", cx=str(width), cy=str(height))
        makeelement(parent, "wp:effectExtent", l="0", r="0", t="0", b="0")
        makeelement(parent, "wp:wrapTopAndBottom")
        self.create_docx_image_markup(
            parent, "cover.jpg", "Cover", img.rid, width, height
        )
        return ans
