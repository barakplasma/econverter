...
from ebook_converter.ebooks.docx.writer.styles import (
...
class SpannedCell(object):
    ...
    def serialize(self, tr, makeelement):
        ...
        makeelement(
            tcPr, "w:%sMerge" % ("h" if self.horizontal else "v"), w_val="continue"
        )
        makeelement(tc, "w:p")
...
class Cell(object):
    ...
    def __init__(self, row, html_tag, tag_style=None):
        ...
        if tag_style is None:
            ...
            self.valign = {"top": "top", "bottom": "bottom", "middle": "center"}.get(
                tag_style._get("vertical-align")
            )
        self.items = []
    ...
    def serialize(self, parent, makeelement):
        ...
        bc = (
            self.background_color
            or self.row.background_color
            or self.row.table.background_color
        )
        if bc:
    ...
    def applicable_borders(self, edge):
        if edge == "left":
            items = (
                {self.table, self.row, self} if self.row.first_cell is self else {self}
            )
        elif edge == "top":
            ...
            items = (
                {self.table, self, self.row} if self.row.last_cell is self else {self}
            )
        elif edge == "bottom":
    ...
    def neighbor(self, edge):
        ...
        if edge == "left":
            ...
            ridx = self.table.rows.index(self.row)
            if ridx + 1 < len(self.table.rows) and idx < len(
                self.table.rows[ridx + 1].cells
            ):
                ans = self.table.rows[ridx + 1].cells[idx]
    ...
    def expand_spanned_cells(self):
        ...
        for row in self.rows:
            for cell in tuple(row.cells):
                ...
                if cell.col_span > 1 and (
                    cell is row.cells[-1]
                    or not isinstance(row.cells[idx + 1], SpannedCell)
                ):
                    row.cells[idx : idx + 1] = [cell] + [
                        SpannedCell(cell, horizontal=True)
                        for i in range(1, cell.col_span)
                    ]
                        ...
                        if tcell is None:
                            nrow.cells.extend(
                                [
                                    ...
                                    for i in range(idx - len(nrow.cells))
                                ]
                            )
