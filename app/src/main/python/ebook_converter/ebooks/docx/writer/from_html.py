...
from ebook_converter.ebooks.docx.writer.container import (
    create_skeleton,
    page_size,
    page_effective_area,
)
...
class TextRun(object):
    ...
    def serialize(self, p, links_manager):
        ...
        parent = (
            p if self.link is None else links_manager.serialize_hyperlink(p, self.link)
        )
        ...
        if self.lang:
            makeelement(
                rpr, "w:lang", w_bidi=self.lang, w_val=self.lang, w_eastAsia=self.lang
            )
        if len(rpr) > 0:
            r.append(rpr)
...
class Block(object):
    def __init__(
        ...
        self.style = styles_manager.create_block_style(
            style, html_block, is_table_cell=is_table_cell, parent_bg=parent_bg
        )
    ...
    def add_text(
        ...
        ts = self.styles_manager.create_text_style(
            style, is_parent_style=is_parent_style
        )
        if (
            self.runs
            and ts == self.runs[-1].style
            and link == self.runs[-1].link
            and lang == self.runs[-1].lang
        ):
            run = self.runs[-1]
        else:
            run = TextRun(
                self.namespace,
                ts,
                self.html_block if html_parent is None else html_parent,
                lang=lang,
            )
    ...
    def add_break(self, clear="none", bookmark=None):
        if self.runs:
            ...
            run = TextRun(
                self.namespace,
                self.styles_manager.create_text_style(self.html_style),
                self.html_block,
            )
            self.runs.append(run)
    ...
    def serialize(self, body):
        ...
        if self.block_lang:
            ...
            makeelement(
                rpr,
                "w:lang",
                w_val=self.block_lang,
                w_bidi=self.block_lang,
    ...
    def end_current_block(self):
        if self.current_block is not None:
            ...
            if (
                ...
                and self.current_table.current_row is not None
            ):
    ...
    def finish_tag(self, html_tag):
        if self.current_block is not None and html_tag in self.open_html_blocks:
            ...
            if (
                ...
                and start_block.html_style["page-break-after"] == "always"
            ):
    ...
    def apply_page_break_after(self):
        for i, block in enumerate(self.all_blocks):
            if block.page_break_after and i < len(self.all_blocks) - 1:
                ...
                if (
                    next_block.parent_items is block.parent_items
                    and block.parent_items is self.items
                ):
...
class Convert(object):
    ...
    def __call__(self):
        ...
        self.styles_manager = StylesManager(
            self.docx.namespace, self.log, self.mi.language
        )
        self.links_manager = LinksManager(
            self.docx.namespace, self.docx.document_relationships, self.log
        )
        self.images_manager = ImagesManager(
            self.oeb, self.docx.document_relationships, self.opts
        )
        self.lists_manager = ListsManager(self.docx)
        self.fonts_manager = FontsManager(self.docx.namespace, self.oeb, self.opts)
        self.blocks = Blocks(
            self.docx.namespace, self.styles_manager, self.links_manager
        )
        ...
        if (
            self.add_cover
            and self.oeb.metadata.cover
            and str(self.oeb.metadata.cover[0]) in self.oeb.manifest.ids
        ):
        ...
        if self.cover_img is not None:
            self.cover_img = self.images_manager.create_cover_markup(
                self.cover_img,
                self.opts.preserve_cover_aspect_ratio,
                *page_size(self.opts),
            )
    ...
    def process_tag(self, html_tag, stylizer, is_first_tag=False, float_spec=None):
        ...
        ignore_tag_contents = (
            tagname in {"script", "style", "title", "meta"} or tag_style.is_hidden
        )
        ...
        if not ignore_tag_contents:
            ...
            if (
                display in {"inline", "inline-block"} or tagname == "br"
            ):  # <br> has display:block but we dont want to start a new paragraph
                if is_float and float_spec.is_dropcaps:
                    self.add_block_tag(
                        tagname, html_tag, tag_style, stylizer, float_spec=float_spec
                    )
                    float_spec = None
                else:
                ...
                self.add_block_tag(
                    tagname, html_tag, tag_style, stylizer, is_list_item=True
                )
            elif display.startswith("table") or display == "inline-table":
                if display == "table-cell":
                    self.blocks.start_new_cell(html_tag, tag_style)
                    self.add_block_tag(
                        tagname, html_tag, tag_style, stylizer, is_table_cell=True
                    )
        ...
        if (
            not is_first_tag
            and html_tag.tail
            and (not ignore_whitespace_tail or not html_tag.tail.isspace())
        ):
    ...
    def add_block_tag(
        ...
        if tagname == "img":
            ...
            if text:
                ...
                tagname == "li"
                and len(html_tag)
                and parse_utils.barename(html_tag[0].tag) in ("ul", "ol")
                and len(html_tag[0])
            ):
    ...
    def add_inline_tag(self, tagname, html_tag, tag_style, stylizer):
        ...
        if tagname == "br":
            if (
                html_tag.tail
                or html_tag is not tuple(html_tag.getparent().iterchildren("*"))[-1]
            ):
                block = self.create_block_from_parent(html_tag, stylizer)
                block.add_break(
                    clear={"both": "all", "left": "left", "right": "right"}.get(
                        tag_style["clear"], "none"
                    ),
                    bookmark=bmark,
                )
        elif tagname == "img":
    ...
    def bookmark_for_anchor(self, anchor, html_tag):
        return self.links_manager.bookmark_for_anchor(
            anchor, self.current_item, html_tag
        )
    ...
    def write(self):
        ...
        self.fonts_manager.serialize(
            self.styles_manager.text_styles,
            self.docx.font_table,
            self.docx.embedded_fonts,
            self.docx.fonts,
        )
