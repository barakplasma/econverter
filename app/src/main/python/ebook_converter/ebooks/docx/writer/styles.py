...
def is_dropcaps(html_tag, tag_style):
    return (
        len(html_tag) < 2
        and len(
            etree.tostring(html_tag, method="text", encoding="unicode", with_tail=False)
        )
        < 5
        and tag_style["float"] == "left"
    )
...
class CombinedStyle(object):
    ...
    def serialize(self, styles, normal_style):
        ...
        def w(x):
            return "{%s}%s" % (self.namespace.namespaces["w"], x)
...
class FloatSpec(object):
    def __init__(self, namespace, html_tag, tag_style):
        ...
        if self.is_dropcaps:
            ...
            self.h_space = int(
                20 * max(tag_style["margin-right"], tag_style["margin-left"])
            )
            self.v_space = int(
                20 * max(tag_style["margin-top"], tag_style["margin-bottom"])
            )
    ...
    def serialize(self, block, parent):
        if self.is_dropcaps:
            attrs = dict(
                w_dropCap="drop",
                w_lines=str(self.dropcaps_lines),
                w_wrap="around",
                w_vAnchor="text",
                w_hAnchor="text",
            )
        ...
        self.makeelement(
            parent, "w:ind", w_left="0", w_leftChars="0", w_right="0", w_rightChars="0"
        )
        attrs = {}
...
class DOCXStyle(object):
    ...
    def makeelement(self, parent, name, **attrs):
        return parent.makeelement(
            self.w(name), **{self.w(k): v for k, v in attrs.items()}
        )
    ...
    def __repr__(self):
        return etree.tostring(
            self.serialize(
                etree.Element(
                    self.__class__.__name__, nsmap={"w": self.namespace.namespaces["w"]}
                )
            ),
            pretty_print=True,
        )
...
class TextStyle(DOCXStyle):
    ...
    def __init__(self, namespace, css, is_parent_style=False):
        ...
        try:
            self.font_size = max(
                0, int(float(css["font-size"]) * 2)
            )  # stylizer normalizes all font sizes into pts
        except (ValueError, TypeError, AttributeError):
            self.font_size = None

        fw = css["font-weight"]
        self.bold = (fw.lower() if hasattr(fw, "lower") else fw) in {
            "bold",
            "bolder",
        } or int_or_zero(fw) >= 700
        self.italic = css["font-style"].lower() in {"italic", "oblique"}
        self.color = convert_color(css["color"])
        self.background_color = (
            None if is_parent_style else convert_color(css.backgroundColor)
        )
        ...
        self.text_transform = css[
            "text-transform"
        ]  # TODO: If lowercase or capitalize, transform the actual text
    ...
    def serialize_properties(self, rPr, normal_style):
        ...
        if is_normal_style or self.font_family != normal_style.font_family:
            rPr.append(
                makeelement(
                    rPr,
                    "rFonts",
                    **{k: self.font_family for k in "ascii cs eastAsia hAnsi".split()},
                )
            )

        for name, attr, vmap in (
            ("sz", "font_size", str),
            ("b", "bold", bmap),
            ("i", "italic", bmap),
        ):
        ...
        if check_attr("underline"):
            rPr.append(
                makeelement(rPr, "u", val="single" if self.underline else "none")
            )
        ...
        if is_normal_style:
            rPr.append(
                makeelement(
                    rPr,
                    "vertAlign",
                    val=self.vertical_align
                    if self.vertical_align in {"superscript", "subscript"}
                    else "baseline",
                )
            )
    ...
    def __init__(self, parent_style, child_style):
        ...
        if parent_style.font_family != child_style.font_family:
            add(
                "rFonts",
                **{
                    k: child_style.font_family
                    for k in "ascii cs eastAsia hAnsi".split()
                },
            )
        ...
        for name, attr in (("sz", "font_size"), ("b", "bold"), ("i", "italic")):
            ...
            if pval != cval:
                val = (
                    "on" if attr in {"bold", "italic"} else str(cval)
                )  # bold, italic are toggle properties
                for suffix in ("", "Cs"):
...
def read_css_block_borders(self, css, store_css_style=False):
    for edge in border_edges:
        if css is None:
            ...
            setattr(
                self,
                "border_%s_color" % edge,
                convert_color(css["border-%s-color" % edge]) or "auto",
            )
            setattr(
                self,
                "border_%s_style" % edge,
                LINE_STYLES.get(css["border-%s-style" % edge].lower(), "none"),
            )
            if store_css_style:
                setattr(
                    self,
                    "border_%s_css_style" % edge,
                    css["border-%s-style" % edge].lower(),
                )
...
class BlockStyle(DOCXStyle):
    ...
    def __init__(self, namespace, css, html_block, is_table_cell=False, parent_bg=None):
        ...
        if css is None:
            ...
            self.background_color = (
                None if is_table_cell else convert_color(css["background-color"])
            )
    ...
    def serialize_borders(self, bdr, normal_style):
        ...
        for edge in border_edges:
            ...
            padding = getattr(self, "padding_" + edge)
            if (self is normal_style and padding > 0) or (
                padding != getattr(normal_style, "padding_" + edge)
            ):
    ...
    def serialize_properties(self, pPr, normal_style):
        ...
        for edge, attr in {"top": "before", "bottom": "after"}.items():
            ...
            if css_unit in ("em", "ex"):
                ...
                if (self is normal_style and lines > 0) or getter(self) != getter(
                    normal_style
                ):
        ...
        for edge in ("left", "right"):
            ...
            if css_unit in ("em", "ex"):
                ...
                if (self is normal_style and chars > 0) or getter(self) != getter(
                    normal_style
                ):
        ...
        if css_unit in ("em", "ex"):
            ...
            if css_val >= 0:
                if (
                    self is normal_style and chars > 0
            ...
            if val >= 0:
                if (
                    self is normal_style and val > 0
        ...
        if (
            self is normal_style and self.background_color
...
class StylesManager(object):
    ...
    def create_block_style(
        self, css_style, html_block, is_table_cell=False, parent_bg=None
    ):
        ans = BlockStyle(
            self.namespace,
            css_style,
    ...
    def finalize(self, all_blocks):
        ...
        for i, heading_tag in enumerate(sorted(heading_styles)):
            styles = sorted(
                (smap[k] for k in heading_styles[heading_tag]), key=counts.__getitem__
            )
        ...
        for i, (text_style, count) in enumerate(ds_counts.most_common()):
            ...
            text_style.seq = i
        self.descendant_text_styles = sorted(
            descendant_style_map, key=attrgetter("seq")
        )
