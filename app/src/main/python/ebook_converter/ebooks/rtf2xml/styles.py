...
class Styles:
    ...
    def __tab_bar_func(self, line):
        ...
        try:
            self.__styles_dict["par"][self.__styles_num]["tabs"] += "%s:" % "bar"
            self.__styles_dict["par"][self.__styles_num]["tabs"] += "%s;" % line[20:-1]
        except KeyError:
            self.__enter_dict_entry("tabs", "")
            self.__styles_dict["par"][self.__styles_num]["tabs"] += "%s:" % "bar"
    ...
    def __print_style_table(self):
        ...
        for type in types:
            if type == "par":
                ...
                prefix = "character"
            self.__write_obj.write("mi<tg<open______<%s-styles\n" % prefix)
            ...
            for num in style_numbers:
                ...
                for att in attributes:
                    ...
                    self.__write_obj.write("<%s>%s" % (att, this_value))
                self.__write_obj.write("\n")
    ...
    def __para_style_in_body_func(self, line, type):
        ...
        try:
            ...
            value = None
        if value:
            self.__write_obj.write("cw<ss<%s-style<nu<%s\n" % (prefix, value))
        else:
