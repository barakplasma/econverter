        ...
        # error
            ...
            if the_num > 10:
                self.__write_obj.write(
                    "mi<tg<empty-att_<udef_symbol<num>%s<description>not-in-table\n"
                    % hex_num
                )
                if self.__run_level > 4:
                    ...
                    msg = (
                        'Character "&#x%s;" does not appear to be valid (or is a control character)\n'
                        % token
                    )
                    raise self.__bug_handler(msg)
        ...
        with open_for_write(self.__write_to) as self.__write_obj:
            with open_for_read(self.__file) as read_obj:
                for line in read_obj:
                    ...
                    if action is None:
                        sys.stderr.write(
                            "error no state found in hex_2_utf8", self.__state
                        )
                    action(line)
    ...
    def __text_func(self, line):
        ...
        if self.__current_dict_name in ("Symbol", "Wingdings", "Zapf Dingbats"):
            ...
            if (
                ...
                and self.__current_dict_name
                not in ("Symbol", "Wingdings", "Zapf Dingbats")
            ):
                text = text.upper()
            self.__write_obj.write("tx<nu<__________<%s\n" % text)
