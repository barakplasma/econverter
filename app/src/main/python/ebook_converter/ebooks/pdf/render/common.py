...
PAPER_SIZES = {
    k: globals()[k.upper()]
    for k in ("a0 a1 a2 a3 a4 a5 a6 b0 b1 b2 b3 b4 b5 b6 letter legal").split()
}
...
class Name(str):
    def pdf_serialize(self, stream):
        ...
        buf = (
            chr(x).encode("ascii")
            if 33 < x < 126 and x != sharp
            else "#{:x}".format(x).encode("ascii")
            for x in raw
        )
...
class Dictionary(dict):
    def pdf_serialize(self, stream):
        stream.write(b"<<" + EOL)
        sorted_keys = sorted(
            self, key=lambda x: {"Type": "1", "Subtype": "2"}.get(x, x) + x
        )
...
class Stream(io.BytesIO):
    ...
    def write(self, raw):
        super(Stream, self).write(
            raw if isinstance(raw, bytes) else raw.encode("ascii")
        )
