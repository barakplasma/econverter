...
class TextileMLizer(OEB2HTML):
    ...
    def mlize_spine(self, oeb_book):
        ...
        for item in oeb_book.spine:
            ...
            stylizer = Stylizer(
                item.data, item.href, oeb_book, self.opts, self.opts.output_profile
            )
    ...
    def check_padding(self, style, stylizer):
        ...
        if "padding-left" in style.cssdict() and style["padding-left"] != "auto":
            left_padding_pts = unit_convert(
                style["padding-left"], style.width, style.fontSize, stylizer.profile.dpi
            )
        if "margin-left" in style.cssdict() and style["margin-left"] != "auto":
            left_margin_pts = unit_convert(
                style["margin-left"], style.width, style.fontSize, stylizer.profile.dpi
            )
        ...
        if "padding-right" in style.cssdict() and style["padding-right"] != "auto":
            right_padding_pts = unit_convert(
                style["padding-right"],
                style.width,
                style.fontSize,
        ...
        if "margin-right" in style.cssdict() and style["margin-right"] != "auto":
            right_margin_pts = unit_convert(
                style["margin-right"], style.width, style.fontSize, stylizer.profile.dpi
            )
    ...
    def dump_text(self, elem, stylizer):
        ...
        if (
            ...
            if (
                ...
                and namespace(p.tag) == const.XHTML_NS
                and elem.tail
            ):
                return [elem.tail]
            return [""]
        ...
        if "margin-top" in style.cssdict() and style["margin-top"] != "auto":
            ems = min(
                int(round(float(style.marginTop) / style.fontSize) - 1), self.MAX_EM
            )
            if ems >= 1:
                text.append("\n\n\xa0" * ems)
