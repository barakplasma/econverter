...
class Extract(ODF2XHTML):
        ...
        for img in imgpath(root):
            ...
            if len(div) == 1:
                ...
                if style and not style.endswith(";"):
                    style = style + ";"
                style += (
                    "position:static"  # Ensures position of containing div is static
                )
        ...
        for frm in self.document.topnode.getElementsByType(odFrame):
            try:
                if frm.getAttrNS(odTEXTNS, "anchor-type") == "page":
                    log.warning(
                        "Document has Pictures anchored to Page, will "
            ...
            for frm in self.document.topnode.getElementsByType(odFrame):
                ...
                if frm.getAttribute("name") == mi.odf_cover_frame:
                    ...
                    if len(img) == 1 and img[0].getAttribute("href") == mi.cover:
                        ...
                        log.info("Removed cover image paragraph from document...")
                        break
    ...
    def filter_load(self, odffile, mi, log):
        ...
        if isinstance(odffile, (str, bytes)) or hasattr(
            odffile, "read"
        ):  # Added by Kovid
    ...
    def __call__(self, stream, odir, log):
        ...
        with directory.CurrentDir(odir):
            ...
            opf.create_manifest(
                [
                    (os.path.abspath(os.path.join(r, f2)), None)
