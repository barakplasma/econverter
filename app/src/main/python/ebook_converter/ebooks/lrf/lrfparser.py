...
from ebook_converter.ebooks.lrf.objects import (
    get_object,
    PageTree,
    StyleObject,
    Font,
...
class LRFDocument(LRFMetaFile):
    ...
    def write_files(self):
        for obj in itertools.chain(self.image_map.values(), self.font_map.values()):
    ...
    def to_xml(self, write_files=True):
        ...
        if th:
            ...
            if write_files:
                with open(
                    prefix + "_thumbnail." + self.doc_info.thumbnail_extension, "wb"
                ) as f:
                    f.write(th)
        bookinfo += (
            '<Language reading="">%s</Language>\n'
        ...
        for page_tree in self:
            ...
            for page in page_tree:
                pages += str(page)
            pages += close
        traversed_objects = [int(i) for i in re.findall(r'objid="(\w+)"', pages)] + [
            pt_id
        ]
...
def option_parser():
    ...
    parser.add_option(
        "--output", "-o", default=None, help="Output LRS file", dest="out"
    )
    parser.add_option(
        "--dont-output-resources",
        default=True,
        action="store_false",
        help="Do not save embedded image and font files to disk",
