...
def Book(
    options, logger, font_delta=0, header=None, profile=PRS500_PROFILE, **settings
):
    ...
    ps["textwidth"] = profile.screen_width - (
        options.left_margin + options.right_margin
    )
    ps["textheight"] = (
        profile.screen_height
        - (options.top_margin + options.bottom_margin)
        - profile.fudge
    )
    if header:
        hdr = Header()
        hb = TextBlock(
            textStyle=TextStyle(
                align="foot", fontsize=int(profile.header_font_size * 10)
            ),
    ...
    for family in ["serif", "sans", "mono"]:
        if not fonts[family]:
            ...
            raise ConversionError(
                "Could not find the normal version of the " + family + " font"
            )
    return book, fonts
