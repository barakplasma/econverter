...
ruby_tags = {
    ...
    0xF57A: [
        "emplinetype",
        "W",
        {0: "none", 0x10: "solid", 0x20: "dashed", 0x30: "double", 0x40: "dotted"},
    ],
}
...
class LRFObject(object):
    ...
    def handle_tag(self, tag, stream, tag_map=None):
        ...
        if tag.id in tag_map:
            ...
            raise LRFParseError(
                "Unknown tag in %s: %s" % (self.__class__.__name__, str(tag))
            )
    ...
    def read_stream(self, tag, stream):
        ...
        if self.stream_flags & 0x200 != 0:
            ...
            if key != 0 and key <= 0xF0:
                ...
                key = 0
            if l > 0x400 and (
                isinstance(self, ImageStream)
                or isinstance(self, Font)
                or isinstance(self, SoundStream)
            ):
                l = 0x400
        ...
        if stream.read(2) != b"\x06\xf5":
            print(
                "Warning: corrupted end-of-stream tag at %08X; skipping it"
                % (stream.tell() - 2)
            )
...
class Color(object):
    ...
    def __getitem__(self, i):  # Qt compatible ordering and values
        return (self.r, self.g, self.b, 0xFF - self.a)[
            i
        ]  # In Qt 0xff is opaque while in LRS 0x00 is opaque
...
class Page(LRFStream):
    ...
    evenheader = property(
        fget=lambda self: self._document.objects[self.style.evenheaderid]
    )
    evenfooter = property(
        fget=lambda self: self._document.objects[self.style.evenfooterid]
    )
    oddheader = property(
        fget=lambda self: self._document.objects[self.style.oddheaderid]
    )
    oddfooter = property(
        fget=lambda self: self._document.objects[self.style.oddfooterid]
    )
    ...
    def header(self, odd):
        id = (
            ...
            if odd
            else self._document.objects[self.style_id].evenheaderid
        )
    ...
    def footer(self, odd):
        id = (
            ...
            if odd
            else self._document.objects[self.style_id].evenfooterid
        )
        return self._document.objects[id]
...
class Text(LRFStream):
    ...
    def add_text(self, text):
        ...
        if s:
            ...
            self.content.append(self.entity_pattern.sub(entities.entity_to_unicode, s))
    ...
    def end_para(self, tag, stream):
        ...
        while i > -1:
            if (
                isinstance(self.content[i], Text.TextTag)
                and self.content[i].name == "P"
            ):
                break
            i -= 1
    ...
    def char_button(self, tag, stream):
        self.content.append(
            self.__class__.TextTag("CharButton", attrs={"refobj": tag.dword})
        )
    ...
    def space(self, tag, stream):
        self.content.append(
            self.__class__.TextTag(
                "Space", attrs={"xsize": tag.sword}, self_closing=True
            )
        )
    ...
    def box(self, tag, stream):
        self.content.append(
            self.__class__.TextTag("Box", {"linetype": self.linetype_map[tag.word]})
        )
    ...
    def __str__(self):
        ...
        if len(open_containers) > 0:
            if len(open_containers) == 1:
                ...
                raise LRFParseError(
                    "Malformed text stream %s"
                    % (
                        [
                            i.name
                            for i in open_containers
                            if isinstance(i, Text.TextTag)
                        ],
                    )
                )
        return s
    ...
    def to_html(self):
        ...
        if len(open_containers) > 0:
            raise LRFParseError(
                "Malformed text stream %s"
                % ([i.name for i in open_containers if isinstance(i, Text.TextTag)],)
            )
        return s
...
class Image(LRFObject):
    ...
    encoding = property(
        fget=lambda self: self._document.objects[self.refstream].encoding
    )
        ...
        while stream.tell() < len(self.stream):
            ...
            try:
                self._contents.append(
                    PutObj(self._document.objects, *struct.unpack("<HHI", tag.contents))
                )
            except struct.error:
...
class Font(LRFStream):
    ...
    def __unicode__(self):
        s = (
            '<RegistFont objid="%s" fontfilename="%s" fontname="%s" encoding="TTF" file="%s" />\n'
...
class BookAttr(StyleObject, LRFObject):
    ...
    def __str__(self):
        ...
        doc = self._document
        s += (
            '<BookSetting bindingdirection="%s" dpi="%s" screenwidth="%s" screenheight="%s" colordepth="%s" />\n'
            % (
                self.binding_map[doc.binding],
                doc.dpi,
                doc.width,
                doc.height,
                doc.color_depth,
...
def get_object(document, stream, id, offset, size, scramble_key):
    ...
    if obj_type < len(object_map) and object_map[obj_type] is not None:
        return object_map[obj_type](
            document, stream, obj_id, scramble_key, offset + size - Tag.tags[0][0]
        )
