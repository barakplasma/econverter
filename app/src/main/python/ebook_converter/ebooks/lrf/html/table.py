...
class Cell(object):
    ...
    def text_block_size(self, tb, maxwidth=sys.maxsize, debug=False):
        ...
        for token, attrs in tokens(tb):
            if attrs is None:
                attrs = {}
            font = default_font
            ls = self.pts_to_pixels(
                attrs.get("baselineskip", ts["baselineskip"])
            ...
            if isinstance(token, Plot):
                width, height = (
                    self.pts_to_pixels(token.xsize),
                    self.pts_to_pixels(token.ysize),
                )
                left, right, top, bottom = add_word(
                    width, height, left, right, top, bottom, height, ws
                )
                continue
            ff = attrs.get("fontfacename", ts["fontfacename"])
            fs = attrs.get("fontsize", ts["fontsize"])
            ...
            for word in token.split():
                ...
                left, right, top, bottom = add_word(
                    width, height, left, right, top, bottom, ls, ws
                )
    ...
    def preferred_width(self, debug=False):
        return ceil(
            max(
                [
                    self.text_block_preferred_width(i, debug=debug)
...
class Table(object):
    ...
    def height(self, maxwidth):
        ...
        return (
            sum([row.height(widths) + self.rowpad for row in self.rows]) - self.rowpad
        )
    ...
    def get_widths(self, maxwidth):
        ...
        for i in range(len(widths)):
            ...
            if wp >= 0:
                widths[i] = max(
                    min_widths[i],
                    ceil((wp / 100) * (maxwidth - (cols - 1) * self.colpad)),
                )

        itercount = 0

        while (
            sum(widths) > maxwidth - ((len(widths) - 1) * self.colpad)
            and itercount < 100
        ):
            for i in range(cols):
                widths[i] = (
                    ceil((95 / 100) * widths[i])
                    if ceil((95 / 100) * widths[i]) >= min_widths[i]
                    else widths[i]
                )
            itercount += 1
