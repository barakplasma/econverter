...
MARKUP_MASSAGE = [  # Close <a /> tags
    ...
    (
        ...
        lambda match: re.compile(
            r"%(open)s|%(close)s" % tag_regex("p"), re.IGNORECASE
        ).sub("", match.group()),
    ),
    ...
    (
        re.compile(r"&(\S+?);"),
        functools.partial(
            entities.entity_to_unicode, exceptions=["lt", "gt", "amp", "quot"]
        ),
    ),
    ...
    # BeautifulSoup treats self closing <div> tags as open <div>
    # tags
    (
...
BOOK_DESIGNER = [
    ...
    (
        re.compile(
            r"<h2[^><]*?id=BookTitle[^><]*?(align=)*(?(1)"
            r"(\w+))*[^><]*?>[^><]*?</h2>",
            re.IGNORECASE,
        ),
        lambda match: (
            '<h1 id="BookTitle" align="%s">%s</h1>'
            % (match.group(2) if match.group(2) else "center", match.group(3))
        ),
    ),
    (
        re.compile(
            r"<h2[^><]*?id=BookAuthor[^><]*?(align=)*(?(1)"
            r"(\w+))*[^><]*?>[^><]*?</h2>",
            re.IGNORECASE,
        ),
        lambda match: (
            '<h2 id="BookAuthor" align="%s">%s</h2>'
            % (match.group(2) if match.group(2) else "center", match.group(3))
        ),
    ),
    (
        re.compile(
            r"<span[^><]*?id=title[^><]*?>(.*?)</span>", re.IGNORECASE | re.DOTALL
        ),
        lambda match: '<h2 class="title">%s</h2>' % match.group(1),
    ),
    (
        re.compile(
            r"<span[^><]*?id=subtitle[^><]*?>(.*?)</span>", re.IGNORECASE | re.DOTALL
        ),
...
def tag_regex(tagname):
    """
    Return non-grouping regular expressions that match the opening and closing
    tags for tagname
    """
    return dict(
        open=r"(?:<\s*%(t)s\s+[^<>]*?>|<\s*%(t)s\s*>)" % dict(t=tagname),
...
class HTMLConverter(object):
    ...
    def __init__(self, book, fonts, options, logger, paths):
        ...
        # Memory
        # Temporary files with scaled version of images
        self.scaled_images = {}
        # Temporary files with rotated version of images
        self.rotated_images = {}
        # Keep track of already used textstyles
        self.text_styles = []
        # Keep track of already used blockstyles
        self.block_styles = []
        # Images referenced in the HTML document
        self.images = {}
        # <a name=...> and id elements
        self.targets = {}
        # <a href=...> elements
        self.links = collections.deque()
        self.processed_files = []
        # TOC entries gleaned from semantic information
        self.extra_toc_entries = []
        self.image_memory = []
        self.id_counter = 0
        # Used to remove extra TextBlocks
        self.unused_target_blocks = []
        # Current link level
        self.link_level = 0
        # Used to ensure that duplicate CSS unhandled erros are not reported
        self.memory = []
        # element representing the top of each HTML file in the LRF file
        self.tops = {}
        # Used to figure out when to lstrip
        self.previous_text = ""
        self.stripped_space = ""
        # Used so that <p> tags in <blockquote> elements are handled properly
        self.preserve_block_style = False
        self.avoid_page_break = False
        self.current_page = book.create_page()

        # Styles
        self.blockquote_style = book.create_block_style(
            sidemargin=60, topskip=20, footskip=20
        )
        ...
        # The Book object representing a BBeB book
        self.book = book
        ...
        if self._override_css is not None:
            ...
            match = PAGE_BREAK_PAT.search(src)
        ...
        while len(paths) > 0 and self.link_level <= self.link_levels:
            for path in paths:
                ...
                try:
                    ...
                    # Die on errors in the first level
    ...
    def preprocess(self, raw):
        ...
        if self.baen:
            nmassage.extend(BAEN)

        if self.pdftohtml:
            nmassage.extend(PDFTOHTML)
        if self.book_designer:
            nmassage.extend(BOOK_DESIGNER)
    ...
    def add_file(self, path):
        ...
        self.log.info("Processing %s", path if self.verbose else self.file_name)
        ...
        with open(path, "rb") as f:
            raw = f.read()
        # Bug in pdftohtml that causes it to output invalid UTF-8 files
        if self.pdftohtml:
    ...
    def parse_css(self, style):
        ...
        for sel in re.findall(SELECTOR_PAT, style):
    ...
    def parse_file(self, soup):
        ...
        # May have been removed for a cover image
        if top not in top.parent.contents:
            top = top.parent.contents[0]
        if (
            not top.has_text()
            and top.parent.contents.index(top) == len(top.parent.contents) - 1
        ):
                ...
                if self.current_page and self.current_page.has_text():
                    for c in self.current_page.contents:
                        if isinstance(c, (TextBlock, ImageBlock)):
                            return c
                raise ConversionError("Could not parse file: %s" % self.file_name)
            else:
                try:
    ...
    def create_link(self, children, tag):
        ...
        if para is None:
            raise ConversionError(
                "Failed to parse link %(tag)s %(children)s"
        ...
        return {
            ...
            "in toc": (
                self.link_level == 0
                and not self.use_spine
                and not self.options.no_links_in_toc
            ),
        }

    def get_text(self, tag, limit=None):
        css = self.tag_css(tag)[0]
        if "display" in css and css["display"].lower() == "none":
            return ""
        if "visibility" in css and css["visibility"].lower() == "hidden":
            return ""
    ...
    def process_links(self):
        ...
        outside_links = collections.deque()
    ...
    def add_image_page(self, path):
        if os.access(path, os.R_OK):
            self.end_page()
            pwidth = self.profile.screen_width
            pheight = self.profile.screen_height - self.profile.fudge
            ...
            canvas.put_object(
                ib, int((pwidth - width) / 2.0), int((pheight - height) / 2.0)
            )
            page.append(canvas)
    ...
    def process_alignment(self, css):
        ...
        if align != self.current_block.textStyle.attrs["align"]:
            ...
            self.current_block = self.book.create_text_block(
                blockStyle=self.current_block.blockStyle, textStyle=ts
            )
            self.current_para = Paragraph()
            return True
        return False
            ...
            def valigner(x):
                if "vertical-align" in css:
                    ...
                    if valign in ("sup", "super", "sub"):
                        ...
                        return Sub(x) if valign == "sub" else Sup(x)
                return x
    ...
    def process_image(
        self, path, tag_css, width=None, height=None, dropcaps=False, rescale=False
    ):
        ...
        def scale_image(width, height):
            ...
            pt = PersistentTemporaryFile(
                suffix="_html2lrf_scaled_image_." + encoding.lower()
            )
            self.image_memory.append(pt)  # Neccessary, trust me ;-)
            try:
                im.resize((int(width), int(height)), PILImage.LANCZOS).save(
                    pt, encoding
                )
                pt.close()
                self.scaled_images[path] = pt
                return pt.name
            # PIL chokes on interlaced PNG images as well a some GIF images
            except (IOError, SystemError) as err:
                self.log.warning("Unable to process image %s. Error: %s", path, err)
        ...
        if dropcaps:
            ...
            lines = int(math.ceil(height / line_height))
            dc = DropCaps(lines)
            dc.append(
                Plot(
                    im,
                    xsize=math.ceil(width * factor),
                    ysize=math.ceil(height * factor),
                )
            )
        ...
        if self.autorotation and width > pwidth and width > height:
            pt = PersistentTemporaryFile(
                suffix="_html2lrf_rotated_image_." + encoding.lower()
            )
            try:
                im = im.rotate(90)
                im.save(pt, encoding)
                ...
                width, height = im.size
            # PIL chokes on interlaced PNG files and since auto-rotation is
            # not critical we ignore the error
            except IOError:
                self.log.debug("Unable to process interlaced PNG %s", original_path)
            finally:
                pt.close()
        ...
        if max(width, height) <= min(pwidth, pheight) / 5:
        ...
        elif height <= int(math.floor((2 / 3) * pheight)):
            pb = self.current_block
            self.end_current_para()
            self.process_alignment(tag_css)
            self.current_para.append(
                Plot(im, xsize=width * factor, ysize=height * factor)
            )
            self.current_block.append(self.current_para)
            self.current_page.append(self.current_block)
            self.current_block = self.book.create_text_block(
            ...
            self.end_page()
            if (
                len(self.current_page.contents) == 1
                and not self.current_page.has_text()
            ):
                self.current_page.contents[0:1] = []
            self.current_page.append(Canvas(width=pwidth, height=height))
            left = int(math.floor((pwidth - width) / 2))
    ...
    def process_page_breaks(self, tag, tagname, tag_css):
        ...
        if (
            ...
            and tag.has_attr(self.force_page_break_attr[1])
    ...
    def block_properties(self, tag_css):
        ...
        ans["footskip"] = set(
            self.book.defaultBlockStyle.attrs["footskip"], s1[2], s2[2]
        )
        ans["sidemargin"] = set(
            self.book.defaultBlockStyle.attrs["sidemargin"], s1[3], s2[3]
        )

        factor = 0.7
        if 2 * int(ans["sidemargin"]) >= factor * int(
            self.current_block.blockStyle.attrs["blockwidth"]
        ):
            # Try using (left + right)/2
            val = int(ans["sidemargin"])
            ans["sidemargin"] = set(
                self.book.defaultBlockStyle.attrs["sidemargin"], s1[1], s2[1]
            )
            val += int(ans["sidemargin"])
            val /= 2.0
        ...
        if 2 * int(ans["sidemargin"]) >= factor * int(
            ...
            ans["sidemargin"] = int(
                (factor * int(self.current_block.blockStyle.attrs["blockwidth"])) / 2
            )
    ...
    def font_properties(self, css):
        """
        Convert the font propertiess in css to the Xylog equivalents. If the
        CSS does not contain a particular font property, the default from
        self.book.defaultTextSytle is used. Assumes 1em = 10pt
        @return: dict, key, variant. The dict contains the Xlog equivalents.
                 key indicates the font type (i.e. bold, bi, normal) and
                 variant is None or 'small-caps'
        """
    ...
    def unit_convert(self, val, pts=False, base_length="10pt"):
        ...
        @return: The number of pixels (an int) if successful. Otherwise,
                 returns None.
        """
    ...
    def text_properties(self, tag_css):
        ...
        if "text-indent" in tag_css:
            ...
            if "em" in tag_css["text-indent"]:
                bl = "10pt"
            indent = self.unit_convert(
                str(tag_css["text-indent"]), pts=True, base_length=bl
            )
            if not indent:
                indent = 0
        ...
        if "line-height" in tag_css:
            bls = int(self.book.defaultTextStyle.attrs["baselineskip"])
            ls = int(self.book.defaultTextStyle.attrs["linespace"])
            try:  # See if line-height is a unitless number
                val = int(float(tag_css["line-height"].strip()) * (ls))
                fp["linespace"] = val
            except ValueError:
                val = self.unit_convert(
                    tag_css["line-height"], pts=True, base_length="1pt"
                )
            if val is not None:
                val -= bls
                if val >= 0:
    ...
    def process_block(self, tag, tag_css):
        ...
        # Since we're flattening the block structure, we need to ensure that
        # text doesn't go off the left edge of the screen
    ...
    def parse_tag(self, tag, parent_css):
        try:
            ...
            if not isinstance(tag, IGNORED_TAGS):
        ...
        try:  # Skip element if its display attribute is set to none
            if (
                tag_css["display"].lower() == "none"
                or tag_css["visibility"].lower() == "hidden"
            ):
                return
        except KeyError:
            pass
        if not self.disable_chapter_detection and (
            ...
            and (
                ...
                or (
                    tag.has_attr(self.chapter_attr[1])
                    and self.chapter_attr[2].match(tag[self.chapter_attr[1]])
                )
            )
        ):
            ...
            if self.options.add_chapters_to_toc:
                self.current_block.must_append = True
                self.extra_toc_entries.append(
                    (self.get_text(tag, limit=1000), self.current_block)
                )
        ...
        try:
            if tagname in ["title", "script", "meta", "del", "frameset"]:
                ...
                if tag.has_attr("href") and not self.link_exclude.match(tag["href"]):
                    if urllib.parse.urlparse(tag["href"])[0] not in ("", "file"):
                        self.process_children(tag, tag_css, tag_pseudo_css)
                    else:
                        ...
                        if os.access(path, os.R_OK) and os.path.isfile(path):
                            if ext in ["png", "jpg", "bmp", "jpeg"]:
                                ...
                                self.links.append(
                                    self.create_link(self.current_para.contents, tag)
                                )
                                if tag.has_attr("id") or tag.has_attr("name"):
                                    key = "name" if tag.has_attr("name") else "id"
                                    self.targets[self.target_prefix + tag[key]] = (
                                        self.current_block
                                    )
                                    self.current_block.must_append = True
                        else:
                            self.log.debug("Could not follow link to %s", tag["href"])
                            self.process_children(tag, tag_css, tag_pseudo_css)
                ...
                if tag.has_attr("src"):
                    ...
                    if os.access(path, os.R_OK) and os.path.isfile(path):
                        ...
                        self.log.warning("Could not find image: %s", tag["src"])
                else:
                ...
                if tagname == "style":
                    ...
                    try:
                        ...
                        if match and not re.match(
                            "avoid", match.group(1), re.IGNORECASE
                        ):
                            self.page_break_found = True
                        ncss, npcss = self.parse_css(src)
                    except IOError:
                        self.log.warning("Could not read stylesheet: %s", tag["href"])
                if ncss:
                ...
                self.current_block = self.book.create_text_block(
                    blockStyle=bs, textStyle=self.unindented_style
                )
                self.process_children(tag, tag_css, tag_pseudo_css)
                self.end_current_block()
                ...
                bs.attrs["sidemargin"] = 60
                bs.attrs["topskip"] = 20
                bs.attrs["footskip"] = 20
                try:
                    index = self.block_styles.index(bs)
                    bs = self.block_styles[index]
                except ValueError:
                ...
                if (self.anchor_ids and tag.has_attr("id")) or (
                    self.book_designer and tag.get("class") in ("title", ["title"])
                ):
                    ...
                    if self.book_designer and tag.get("class") in ("title", ["title"]):
                        self.extra_toc_entries.append(
                            (self.get_text(tag, 100), self.current_block)
                        )
                ...
                self.end_current_block()
                if tagname == "hr" and not tag_css.get("width", "").strip().startswith(
                    "0"
                ):
                    (
                        self.current_page.RuledLine(
                            linelength=int(
                                self.current_page.pageStyle.attrs["textwidth"]
                            )
                        )
                    )
                self.previous_text = "\n"
    ...
    def process_table(self, tag, tag_css):
        ...
        for block, xpos, ypos, delta, targets in table.blocks(
            int(ps["textwidth"]), int(ps["textheight"])
        ):
            if not block:
                ...
                canvases.append(
                    Canvas(
                        int(self.current_page.pageStyle.attrs["textwidth"]),
                        ypos + rowpad,
    ...
    def cleanup(self):
        for _file in itertools.chain(
            self.scaled_images.values(), self.rotated_images.values()
        ):
            _file.__del__()
...
def process_file(path, options, logger):
    ...
    if getattr(options, "cover", None):
        ...
        if os.access(options.cover, os.R_OK):
            ...
            pwidth = options.profile.screen_width
            pheight = options.profile.screen_height - options.profile.fudge
            width, height = im.size
            if width < pwidth:
                corrf = pwidth / width
            ...
            try:
                cim = (
                    im.resize((width, height), PILImage.BICUBIC).convert("RGB")
                    if scaled
                    else im
                )
                ...
                tim = im.resize((int(0.75 * th), th), PILImage.LANCZOS).convert("RGB")
    ...
    pb = (
        re.compile(options.page_break, re.IGNORECASE)
        if options.page_break
        else re.compile("$")
    )
    fpb = (
        re.compile(options.force_page_break, re.IGNORECASE)
        if options.force_page_break
        else re.compile("$")
    )
    cq = options.chapter_attr.split(",")
    if len(cq) < 3:
        raise ValueError("The --chapter-attr setting must have 2 commas.")
    options.chapter_attr = [
        re.compile(cq[0], re.IGNORECASE),
        cq[1],
    ...
    if len(fpba) != 3:
        fpba = ["$", "", "$"]
    options.force_page_break_attr = [
        re.compile(fpba[0], re.IGNORECASE),
        fpba[1],
    ...
    files = (
        options.spine if (options.use_spine and hasattr(options, "spine")) else [path]
    )
