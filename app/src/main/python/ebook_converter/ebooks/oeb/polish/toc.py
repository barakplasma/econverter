...
from ebook_converter.utils.localization import (
    get_lang,
    canonicalize_lang,
    lang_as_iso639_1,
)
    ...
    def remove_duplicates(self, only_text=True):
        ...
        for child in self:
            key = (
                child.title
                if only_text
                else (child.title, child.dest, (child.frag or None))
            )
            if key in seen:
                remove.append(child)
            else:
...
def add_from_navpoint(container, navpoint, parent, ncx_name):
    ...
    if nl:
        ...
        for txt in child_xpath(nl, "text"):
            text += etree.tostring(
                txt, method="text", encoding="unicode", with_tail=False
            )
...
def parse_ncx(container, ncx_name):
    ...
    for attr, val in root.attrib.items():
        if attr.endswith("lang"):
            ...
            break
    for uid in root.xpath(
        '//*[calibre:lower-case(local-name()) = "meta" and @name="dtb:uid"]/@content'
    ):
        if uid:
    ...
    for pl in root.xpath('//*[calibre:lower-case(local-name()) = "pagelist"]'):
        for pt in pl.xpath(
            'descendant::*[calibre:lower-case(local-name()) = "pagetarget"]'
        ):
            pagenum = pt.get("value")
            if pagenum:
                href = pt.xpath(
                    'descendant::*[calibre:lower-case(local-name()) = "content"]/@src'
                )
                if href:
                    dest = container.href_to_name(href[0], base=ncx_name)
                    frag = urllib.parse.urlparse(href[0]).fragment or None
                    toc_root.page_list.append(
                        {"dest": dest, "pagenum": pagenum, "frag": frag}
                    )
    return toc_root
...
def add_from_li(container, li, parent, nav_name):
    dest = frag = text = None
    for x in li.iterchildren(base.tag("xhtml", "a"), base.tag("xhtml", "span")):
        text = (
            etree.tostring(
                x, method="text", encoding="unicode", with_tail=False
            ).strip()
        ...
        if href:
            dest = (
                nav_name
                if href.startswith("#")
                else container.href_to_name(href, base=nav_name)
            )
            frag = urllib.parse.urlparse(href).fragment or None
        break
    return parent.add(text or None, dest or None, frag or None)
        ...
        if nav.get(base.tag("epub", "type")) == "toc":
            ...
            if ol is not None:
                ...
                for h in nav.iterchildren(*map(xhtml, "h1 h2 h3 h4 h5 h6".split())):
                    text = etree.tostring(
                        h, method="text", encoding="unicode", with_tail=False
                    ) or h.get("title")
                    if text:
...
def verify_toc_destinations(container, toc):
    ...
    for item in toc.iterdescendants():
        ...
        if not item.dest_exists:
            item.dest_error = "The anchor %(a)s does not exist in file %(f)s" % dict(
                a=item.frag, f=name
            )
                    ...
                    for a in li.iterdescendants(base.tag("xhtml", "a")):
                        ...
                        if href:
                            title = etree.tostring(
                                a, method="text", encoding="unicode", with_tail=False
                            ).strip()
                            href, frag = href.partition("#")[::2]
...
def from_xpaths(container, xpaths):
    ...
    for spinepath in container.spine_items:
        ...
        level_item_map = maps[name] = {
            i + 1: frozenset(xp(root)) for i, xp in enumerate(xpaths)
        }
    ...
    if empty_levels:
        for name, lmap in tuple(maps.items()):
            lmap = {
                lvl: items for lvl, items in lmap.items() if lvl not in empty_levels
            }
    ...
    def parent_for_level(child_level):
        ...
        def process_node(node):
            ...
            return (
                node if lvl > limit else child if lvl == limit else process_node(child)
            )
...
def create_ncx(toc, to_href, btitle, lang, uid):
    ...
    ncx = etree.Element(
        base.tag("ncx", "ncx"),
        attrib={"version": "2005-1", base.tag("xml", "lang"): lang},
        nsmap={None: const.NCX_NS},
    )
    head = etree.SubElement(ncx, base.tag("ncx", "head"))
    etree.SubElement(head, base.tag("ncx", "meta"), name="dtb:uid", content=str(uid))
    etree.SubElement(
        head, base.tag("ncx", "meta"), name="dtb:depth", content=str(toc.depth)
    )
    generator = "".join(["calibre (", constants_old.__version__, ")"])
    etree.SubElement(
        head, base.tag("ncx", "meta"), name="dtb:generator", content=generator
    )
    etree.SubElement(
        head, base.tag("ncx", "meta"), name="dtb:totalPageCount", content="0"
    )
    etree.SubElement(
        head, base.tag("ncx", "meta"), name="dtb:maxPageNumber", content="0"
    )
...
def ensure_single_nav_of_type(root, ntype="toc"):
    et = base("epub", "type")
    navs = [
        n for n in root.iterdescendants(base.tag("xhtml", "nav")) if n.get(et) == ntype
    ]
    for x in navs[1:]:
        extract(x)
    if navs:
...
def commit_nav_toc(container, toc, lang=None, landmarks=None, previous_nav=None):
    ...
    if tocname is None:
        ...
        if previous_nav is not None:
            ...
            root = container.parse_xhtml(
                (
                    importlib.resources.files("ebook_converter") / "data/new_nav.html"
                ).read_text()
            )
        container.replace(tocname, root)
    else:
    ...
    def process_node(xml_parent, toc_parent):
        for child in toc_parent:
            ...
            a = li.makeelement(base.tag("xhtml", "a" if child.dest else "span"))
            a.text = title
            li.append(a)
            if child.dest:
                href = to_href(child.dest)
    ...
    if landmarks is not None:
        ...
        for entry in landmarks:
            if (
                entry["type"]
                and container.has_name(entry["dest"])
                and container.mime_map[entry["dest"]] in base.OEB_DOCS
            ):
...
def find_inline_toc(container):
    for name, linear in container.spine_names:
        if container.parsed(name).xpath(
            '//*[local-name()="body" and @id="calibre_generated_inline_toc"]'
        ):
            return name
...
def toc_to_html(toc, container, toc_name, title, lang=None):
    ...
    css_f = (
        importlib.resources.files("ebook_converter") / "data/inline_toc_styles.css"
    ).read_text()
    html = E.html(
        E.head(E.title(title), E.style(css_f, type="text/css")),
        E.body(E.h2(title), E.ul(), id="calibre_generated_inline_toc"),
    )
