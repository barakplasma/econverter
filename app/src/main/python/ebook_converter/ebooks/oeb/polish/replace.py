...
from ebook_converter.ebooks.oeb.polish.css import (
    iter_declarations,
...
def replace_links(
    container, link_map, frag_map=lambda name, frag: frag, replace_in_opf=False
):
    """
...
def rename_files(container, file_map):
    ...
    if overlap:
        raise ValueError(
            "Circular rename detected. The files %s are both rename targets and destinations"
            % ", ".join(overlap)
        )
    for name, dest in file_map.items():
        if container.exists(dest):
            if name != dest and name.lower() == dest.lower():
                ...
                continue
            raise ValueError(
                "Cannot rename {0} to {1} as {1} already exists".format(name, dest)
            )
    if len(file_map.values()) != len(set(file_map.values())):
        raise ValueError(
            "Cannot rename, the set of destination files contains duplicates"
        )
    link_map = {}
            ...
            for itemid, q in container.manifest_id_map.items():
                if q == nname:
                    for item in container.opf_xpath(
                        '//opf:manifest/opf:item[@href and @id="%s"]' % itemid
                    ):
...
def get_recommended_folders(container, names):
    ...
    return {
        n: recommendations.get(
            mt_to_category(container, guess_type(os.path.basename(n))), opf_folder
        )
        for n in names
    }
...
def normalize_case(container, val):
    ...
    for i in range(len(parts)):
        ...
        candidates = [
            c
            for c in safe_listdir(os.path.dirname(x))
            if c != parts[i] and c.lower() == xl
        ]
            ...
            for tag in stylepath(root):
                if tag.text and (tag.get("type") or "text/css").lower() == "text/css":
                    ...
                    if remove_links_in_sheet(
                        partial(container.href_to_name, base=name), sheet, predicate
                    ):
            ...
            for tag in styleattrpath(root):
                ...
                if style:
                    ...
                    if remove_links_in_declaration(
                        partial(container.href_to_name, base=name), style, predicate
                    ):
                        removed = True
