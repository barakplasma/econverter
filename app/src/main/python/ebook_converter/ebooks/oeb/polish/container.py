...
from ebook_converter.customize.ui import (
...
class ContainerBase(object):  # {{{
    ...
    def decode(self, data, normalize_to_nfc=True):
        ...
        if data[:4] in {b"\0\0\xfe\xff", b"\xff\xfe\0\0"}:
            bom_enc = {b"\0\0\xfe\xff": "utf-32-be", b"\xff\xfe\0\0": "utf-32-le"}[
                data[:4]
            ]
            data = data[4:]
    ...
    def parse_xhtml(self, data, fname="<string>", force_html5_parse=False):
        if self.tweak_mode:
            return parse_html_tweak(
                ...
                log=self.log,
                decoder=self.decode,
                force_html5_parse=force_html5_parse,
            )
        else:
            try:
                return parse_html(
...
class Container(ContainerBase):  # {{{
    ...
    def __init__(self, rootpath, opfpath, log, clone_data=None):
        ...
        self.root = (
            clone_data["root"] if clone_data is not None else os.path.abspath(rootpath)
        )
    ...
    def add_name_to_manifest(self, name, process_manifest_item=None):
        ...
        item = manifest.makeelement(oeb_base.tag("opf", "item"), id=item_id, href=href)
    ...
    def manifest_has_name(self, name):
        ...
        all_names = {
            self.href_to_name(x.get("href"), self.opf_name)
    ...
    def add_file(
        ...
        if self.has_name_case_insensitive(name) or self.manifest_has_name(name):
            if not modify_name_if_needed:
                raise ValueError(
                    ("A file with the name %s already exists" % name)
        ...
        if self.ok_to_be_unmanifested(name):
            return name
        item_id = self.add_name_to_manifest(
            name, process_manifest_item=process_manifest_item
        )
    ...
    def rename(self, current_name, new_name):
        ...
        if self.exists(new_name) and (
            new_name == current_name or new_name.lower() != current_name.lower()
        ):
        ...
        if os.path.isfile(base):
            raise ValueError(
                "Cannot rename %s to %s as %s is a file"
    ...
    def iterlinks(self, name, get_line_numbers=True):
        ...
        if name == self.opf_name:
            for elem in self.opf_xpath("//*[@href]"):
                yield (
                    (elem.get("href"), elem.sourceline, 0)
                    if get_line_numbers
                    else elem.get("href")
                )
        elif media_type.lower() in oeb_base.OEB_DOCS:
            ...
            for elem in self.parsed(name).xpath("//*[@src]"):
                yield (
                    (elem.get("src"), elem.sourceline, 0)
                    if get_line_numbers
                    else elem.get("src")
                )
    ...
    def abspath_to_name(self, fullpath, root=None):
        ...
        return unicodedata.normalize(
            "NFC", abspath_to_name(fullpath, root or self.root)
        )
    ...
    def href_to_name(self, href, base=None):
        ...
        if ans is null:
            ans = self.href_to_name_cache[key] = href_to_name(
                href, self.root, base=base
            )
        return ans
    ...
    def raw_data(self, name, decode=True, normalize_to_nfc=True):
        ...
        if decode and (
            mime in oeb_base.OEB_STYLES
            or mime in oeb_base.OEB_DOCS
            or mime == "text/plain"
            or mime[-4:] in {"+xml", "/xml"}
        ):
    ...
    def manifest_type_map(self):
        ...
        for item in self.opf_xpath("//opf:manifest/opf:item[@href and @media-type]"):
            ans[item.get("media-type").lower()].append(
    ...
    def set_spine(self, spine_items):
        ...
        tail, last_tail = (
            (items[0].tail, items[-1].tail) if items else ("\n    ", "\n  ")
        )
        ...
        for name, linear in spine_items:
            i = spine.makeelement(
                oeb_base.tag("opf", "itemref"), nsmap={"opf": const.OPF2_NS}
            )
            i.tail = tail
    ...
    def format_opf(self):
        try:
            ...
            for child in mdata:
                child.tail = "\n    "
                try:
                    if child.get("name", "").startswith("calibre:") and child.get(
                        "content", ""
                    ).strip() in {"{}", ""}:
                        remove.add(child)
    ...
    def serialize_item(self, name):
        ...
        if name == self.opf_name:
            self.format_opf()
        data = oeb_base.serialize(
            data, self.mime_map[name], pretty_print=name in self.pretty_print
        )
    ...
    def get_file_path_for_processing(self, name, allow_modification=True):
        ...
        if not os.path.exists(base):
            ...
            if (
                self.cloned
                and allow_modification
                and os.path.exists(path)
                and nlinks_file(path) > 1
            ):
                # Decouple this file from its links
                temp = path + "xxx"
    ...
    def open(self, name, mode="rb"):
        ...
        return open(
            self.get_file_path_for_processing(name, mode not in {"r", "rb"}), mode
        )
    ...
    for dirpath, dirnames, filenames in os.walk(basedir):
        for vcsdir in VCS_DIRS:
            try:
                ...
                pass
        is_root = os.path.abspath(os.path.normcase(dirpath)) == os.path.abspath(
            os.path.normcase(basedir)
        )
...
class EpubContainer(Container):
    ...
    def __init__(self, pathtoepub, log, clone_data=None, tdir=None):
        ...
        if self.is_dir:
            ...
            with open(self.pathtoepub, "rb") as stream:
                try:
                    ...
                    log.exception(
                        "EPUB appears to be invalid ZIP file, "
        ...
        opf_path = os.path.join(
            self.root, *(urllib.parse.unquote(opf_files[0].get("full-path")).split("/"))
        )
        if not exists(opf_path):
            raise InvalidEpub(
                "OPF file does not exist at location pointed to"
    ...
    @property
    def names_that_need_not_be_manifested(self):
        return super(EpubContainer, self).names_that_need_not_be_manifested | {
            "META-INF/" + x for x in self.META_INF
        }

    def ok_to_be_unmanifested(self, name):
        return name in self.names_that_need_not_be_manifested or name.startswith(
            "META-INF/"
        )

    @property
    def names_that_must_not_be_removed(self):
        return super(EpubContainer, self).names_that_must_not_be_removed | {
            "META-INF/container.xml"
        }

    @property
    def names_that_must_not_be_changed(self):
        return super(EpubContainer, self).names_that_must_not_be_changed | {
            "META-INF/" + x for x in self.META_INF
        }
        ...
        if name in self.obfuscated_fonts:
            ...
            for em in enc.xpath('//*[local-name()="EncryptionMethod" and @Algorithm]'):
                ...
                if name == self.href_to_name(cr.get("URI")):
                    ...
                    self.dirty("META-INF/encryption.xml")
        super(EpubContainer, self).remove_item(
            name, remove_from_guide=remove_from_guide
        )
                ...
                if fname is not None:
                    ...
                    base = (
                        ...
                        if is_root
                            ...
                            self.root, os.path.relpath(dirpath, self.pathtoepub)
                        )
                    )
            ...
            for dirpath, dirnames, filenames in os.walk(self.root):
                ...
                base = (
                    ...
                    if is_root
                        ...
                        self.pathtoepub, os.path.relpath(dirpath, self.root)
                    )
                )
                try:
                    os.mkdir(base)
                ...
                for fname in filenames:
                    with (
                        open(os.path.join(dirpath, fname), "rb") as src,
                        open(os.path.join(base, fname), "wb") as dest,
                    ):
...
class AZW3Container(Container):
    ...
    def __init__(self, pathtoazw3, log, clone_data=None, tdir=None):
        ...
        with open(pathtoazw3, "rb") as stream:
            raw = stream.read(3)
            if raw == b"TPZ":
                raise InvalidMobi("This is not a MOBI file. It is a Topaz file.")
            ...
            if kf8_type is None:
                raise InvalidMobi(
                    "This MOBI file does not contain a KF8 "
            ...
            if kf8_type == "joint":
                raise InvalidMobi(
                    "This MOBI file contains both KF8 and older "
        ...
        try:
            opf_path, obfuscated_fonts = fork_job(
                "ebook_converter.ebooks.oeb.polish.container",
                "do_explode",
...
def get_container(path, log=None, tdir=None, tweak_mode=False):
    ...
    ebook = (
        AZW3Container
        if path.rpartition(".")[-1].lower()
        in {"azw3", "mobi", "original_azw3", "original_mobi"}
        and not isdir
        else EpubContainer
    )(path, log, tdir=tdir)
