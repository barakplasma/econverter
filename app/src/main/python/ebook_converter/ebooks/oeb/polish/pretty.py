...
def pretty_opf(root):
    ...
    spine_ids = root.xpath(
        "//opf:spine/opf:itemref/@idref", namespaces=const.OPF_NAMESPACES
    )
...
def pretty_block(parent, level=1, indent="  "):
    ...
    if not parent.text or isspace(parent.text):
        parent.text = ""
    if hasattr(parent.tag, "strip") and parse_utils.barename(parent.tag) in {
        "tr",
        "td",
        "th",
    }:
...
def pretty_script_or_style(container, child):
    if child.text:
        ...
        child.text = "\n" + "\n".join(
            [(indent + x) if x else "" for x in child.text.splitlines()]
        )
