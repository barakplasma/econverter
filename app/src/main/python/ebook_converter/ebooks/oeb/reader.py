...
class OEBReader(object):
    ...
    def _clean_opf(self, opf):
        ...
        for elem in opf.iter(tag=etree.Element):
            if parse_utils.namespace(elem.tag) in (
                "",
                const.OPF1_NS,
        ...
        nroot = etree.Element(
            base.tag("opf", "package"), nsmap={None: const.OPF2_NS}, attrib=attrib
        )
        metadata = etree.SubElement(nroot, base.tag("opf", "metadata"), nsmap=nsmap)
    ...
    def _read_opf(self):
        ...
        data = re.sub(
            r"http://openebook.org/namespaces/oeb-package/1.0(/*)", const.OPF1_NS, data
        )
        try:
            opf = etree.fromstring(data)
        except etree.XMLSyntaxError:
            data = entities.xml_replace_entities(clean_xml_chars(data), encoding=None)
            try:
                opf = etree.fromstring(data)
    ...
    def _metadata_from_opf(self, opf):
        ...
        if not mi.book_producer:
            mi.book_producer = "%(a)s (%(v)s) [http://%(a)s-ebook.com]" % dict(
                a=__appname__, v=__version__
            )
        ...
        for item in list(self.oeb.manifest.values()):
            if item.media_type in check:
                try:
                    ...
                    self.logger.exception("Failed to parse content in %s", item.href)
                    bad.append(item)
    ...
    def _manifest_add_missing(self, invalid):
        ...
        while unchecked:
            ...
            for item in unchecked:
                ...
                if item.media_type in cdoc or item.media_type[-4:] in ("/xml", "+xml"):
                    try:
                        ...
                        self.oeb.log.exception(
                            "Failed to read from manifest entry with id: %s, ignoring",
                            item.id,
                        )
                        invalid.add(item)
                        continue
                ...
                if item.media_type in base.OEB_DOCS or item.media_type[-4:] in (
                    "/xml",
                    "+xml",
                ):
                    ...
                    for href in hrefs:
                        ...
                        try:
                            ...
                            self.oeb.log.exception("Skipping invalid href: %r", href)
                            continue
            ...
            for href in new:
                ...
                if not self.oeb.container.exists(href):
                    if href not in warned:
                        self.logger.warning("Referenced file %r not found", href)
                        warned.add(href)
                    continue
                if href not in warned:
                    self.logger.warning("Referenced file %r not in manifest", href)
                    warned.add(href)
    ...
    def _spine_add_extra(self):
        ...
        while unchecked:
            ...
            for item in unchecked:
                ...
                for href in selector(item.data):
                    ...
                    if (
                        found.media_type not in base.OEB_DOCS
                        or found in spine
                        or found in extras
                    ):
                        continue
        ...
        for item in sorted(extras):
            if item.href in removed_items_to_ignore:
                continue
            if version >= 2:
                self.logger.warning("Spine-referenced file %r not in spine", item.href)
    ...
    def _spine_from_opf(self, opf):
        ...
        for elem in base.xpath(opf, "/o2:package/o2:spine/o2:itemref"):
            ...
            if (
                item.media_type.lower() in base.OEB_DOCS
                ...
                if (
                    ...
                    self.oeb.log.warning(
                        "The item %s is not a XML document. Removing it from spine.",
                        item.href,
                    )
        if len(spine) == 0:
            raise base.OEBError("Spine is empty")
        self._spine_add_extra()
        for val in base.xpath(opf, "/o2:package/o2:spine/@page-progression-direction"):
            if val in {"ltr", "rtl"}:
        ...
        for elem in base.xpath(opf, "/o2:package/o2:guide/o2:reference"):
            ...
            if path not in manifest.hrefs:
                ...
                if corrected_href is None:
                    self.logger.warning("Guide reference %r not found", ref_href)
                    continue
        ...
        for child in children:
            ...
            if descriptionElement:
                description = etree.tostring(
                    descriptionElement[0], method="text", encoding="unicode"
                ).strip()
                if not description:
                    description = None
            else:
        ...
        for href in dups:
            ...
            for x in items:
                if x not in self.oeb.spine:
                    self.oeb.log.warning(
                        "Removing duplicate manifest item with id: %s", x.id
                    )
