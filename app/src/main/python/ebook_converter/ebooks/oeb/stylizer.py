...
from css_parser.css import CSSStyleRule, CSSPageRule, CSSFontFaceRule, cssproperties
from css_parser import (
    profile as cssprofiles,
    parseString,
    parseStyle,
    log as css_parser_log,
    CSSParser,
    profiles,
    replaceUrls,
)
...
from ebook_converter.css_selectors import (
    Select,
    SelectorError,
...
def html_css_stylesheet():
    ...
    if _html_css_stylesheet is None:
        html_css = (
            importlib.resources.files("ebook_converter") / "data/html.css"
...
def media_ok(raw):
    ...
    try:
        for mq in (
            CSSMedia3Parser().parse_stylesheet("@media %s {}" % raw).rules[0].media
        ):
            if query_ok(mq):
                return True
        return False
...
class StylizerRules(object):
        ...
        for sheet_index, stylesheet in enumerate(stylesheets):
            ...
            for rule in stylesheet.cssRules:
                if rule.type == rule.MEDIA_RULE:
                    ...
                    self.rules.extend(
                        self.flatten_rule(
                            rule, href, index, is_user_agent_sheet=sheet_index == 0
                        )
                    )
                    index = index + 1
    ...
    def flatten_style(self, cssstyle):
        ...
        if "font-size" in style:
            ...
            if size in FONT_SIZE_NAMES:
                style["font-size"] = "%.1frem" % (
                    self.profile.fnames[size] / float(self.profile.fbase)
                )
    ...
    def _apply_text_align(self, text):
        if text in ("left", "justify") and self.opts.change_justification in (
            "left",
            "justify",
        ):
...
class Stylizer(object):
    ...
    def __init__(
        ...
        parser = CSSParser(
            fetcher=self._fetch_css_file, log=logging.getLogger("calibre.css")
        )
        for elem in style_tags:
            if (
                elem.tag == base.tag("xhtml", "style")
                    ...
                    for rule in stylesheet.cssRules:
                        if rule.type == rule.IMPORT_RULE:
                            ...
                            if ihref not in hrefs:
                                self.logger.warning(
                                    "Ignoring missing stylesheet in @import rule: %s",
                                    rule.href,
                                )
                                continue
                            sitem = hrefs[ihref]
                            if sitem.media_type not in base.OEB_STYLES:
                                self.logger.warning(
                                    "CSS @import of non-CSS file %r", rule.href
                                )
                                continue
                ...
                elem.tag == base.tag("xhtml", "link")
                and elem.get("href")
                and elem.get("rel", "stylesheet").lower() == "stylesheet"
                and elem.get("type", base.CSS_MIME).lower() in base.OEB_STYLES
                and media_ok(elem.get("media"))
            ):
        ...
        if (
            not hasattr(self.oeb, "stylizer_rules")
        ) or not self.oeb.stylizer_rules.same_rules(
            self.opts, self.profile, stylesheets
        ):
            self.oeb.stylizer_rules = StylizerRules(
                self.opts, self.profile, stylesheets
            )
        ...
        for _, _, cssdict, text, _ in self.rules:
            ...
            try:
                ...
                self.logger.error(
                    "Ignoring CSS rule with invalid selector: %r (%s)", text, err
                )
                continue
                    ...
                    for elem in matches:
                        for x in elem.iter("*"):
                            if x.text:
                                ...
                                span = x.makeelement("{%s}span" % const.XHTML_NS)
        ...
        for elem in base.xpath(tree, "//h:img[@width or @height]"):
            ...
            is_styled = (
                style._style.get("width", "auto") != "auto"
                or style._style.get("height", "auto") != "auto"
            )
            if not is_styled:
    ...
    def stylesheet(self, name, font_scale=None):
        ...
        for _, _, style, selector, href in self.rules:
            ...
            if (
                font_scale
                and "font-size" in style
                and style["font-size"].endswith("pt")
            ):
...
class Style(object):
    ...
    def _get(self, name):
        ...
        if result == "inherit" or (
            result is None and name in INHERITED and self._has_parent()
        ):
    ...
    def _unit_convert(self, value, base=None, font=None):
        ...
        return unit_convert(
            ...
            font,
            self._profile.dpi,
            body_font_size=self._stylizer.body_font_size,
        )
    ...
    def backgroundColor(self):
        ...
        def validate_color(col):
            return cssprofiles.validateWithProfile(
                "color", col, profiles=[profiles.Profiles.CSS_LEVEL_2]
            )[1]
    ...
    def img_size(self, width, height):
        ...
        answ, ansh = (
            self.img_dimension("width", width),
            self.img_dimension("height", height),
        )
    ...
    def is_hidden(self):
        return (
            self._style.get("display") == "none"
            or self._style.get("visibility") == "hidden"
        )
