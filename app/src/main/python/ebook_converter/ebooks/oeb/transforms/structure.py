...
def at_start(elem):
    ...
    for x in body.iter():
        if x is elem:
            return True
        if hasattr(getattr(x, "tag", None), "rpartition") and x.tag.rpartition("}")[
            -1
        ] in {"img", "svg"}:
            return False
        if isspace(getattr(x, "text", None)) and (
            x in ancestors or isspace(getattr(x, "tail", None))
        ):
            continue
        return False
    return False
...
class DetectStructure(object):
    def __call__(self, oeb, opts):
        ...
        if self.oeb.auto_generated_toc or opts.use_auto_toc:
            ...
            if self.oeb.toc.count() < 2 and orig_toc.count() > 2:
                ...
                self.log.info(
                    "Auto generated TOC with %d entries.", self.oeb.toc.count()
                )
        ...
        if opts.page_breaks_before is not None:
            ...
            for item in oeb.spine:
                for elem in pb_xpath(item.data):
                    try:
                        prev = next(
                            elem.itersiblings(tag=etree.Element, preceding=True)
                        )
                        if (
                            parse_utils.barename(elem.tag) in {"h1", "h2"}
                            and parse_utils.barename(prev.tag) in {"h1", "h2"}
    ...
    def detect_start_reading(self):
        ...
        for item in self.oeb.spine:
            ...
            if matches:
                ...
                self.log.info(
                    "Setting start reading at position to %s in %s",
                    self.opts.start_reading_at,
                    item.href,
                )
                return
        self.log.warning(
            "Failed to find start reading at position: %s", self.opts.start_reading_at
        )
    ...
    def detect_chapters(self):
        ...
        def find_matches(expr, doc):
            try:
                ...
                self.log.warning("Invalid chapter expression, ignoring: %s", expr)
                return []

        if self.opts.chapter:
            chapter_path, title_attribute = self.get_toc_parts_for_xpath(
                self.opts.chapter
            )
            ...
            for item, elem in self.detected_chapters:
                ...
                if chapter_mark == "rule":
                    ...
                    if c[item] < 3 and at_start(elem):
                        ...
                        # chapter expression.
                        continue
                    mark = elem.makeelement(
                        base.tag("xhtml", "div"), style=page_break_after
                    )
                else:  # chapter_mark == 'both':
                    mark = elem.makeelement(
                        base.tag("xhtml", "hr"), style=page_break_before
                    )
                try:
    ...
    def create_toc_from_chapters(self):
        ...
        for item, elem in self.detected_chapters:
            text, href = self.elem_to_link(
                item, elem, self.chapter_title_attribute, counter
            )
            ...
            for a in XPath("//h:a[@href]")(item.data):
                ...
                if not purl[0] or purl[0] == "file":
                    ...
                    if not self.oeb.toc.has_href(href):
                        ...
                        text = text[:100].strip()
                        if (
                            not self.opts.duplicate_links_in_toc
                            and self.oeb.toc.has_text(text)
                        ):
                            continue
                        try:
                            self.oeb.toc.add(
                                text, href, play_order=self.oeb.toc.next_play_order()
                            )
                            num += 1
                        except ValueError:
                            self.oeb.log.critical("Failed to process link: %r", href)
                            # Most likely an incorrectly URL encoded link
                            continue
                        if (
                            self.opts.max_toc_links > 0
                            and num >= self.opts.max_toc_links
                        ):
                            self.log.info("Maximum TOC links reached, stopping.")
                            return
    ...
    def add_leveled_toc_items(self):
        ...
        for document in self.oeb.spine:
            ...
            (level1_toc, level1_title) = self.get_toc_parts_for_xpath(
                self.opts.level1_toc
            )
                ...
                for elem in find_matches(level2_toc, document.data):
                    ...
                    for item in document.data.iterdescendants():
                        if item in added:
                            ...
                            text, _href = self.elem_to_link(
                                document, elem, level2_title, counter
                            )
                            counter += 1
                            if text:
                    ...
                    for elem in find_matches(level3_toc, document.data):
                        ...
                        for item in document.data.iterdescendants():
                            if item in added2:
                                ...
                                text, _href = self.elem_to_link(
                                    document, elem, level3_title, counter
                                )
                                counter += 1
                                if text:
                                    level2.add(
