...
class HTMLTOCAdder(object):
    ...
    def __call__(self, oeb, context):
        ...
        if "toc" in oeb.guide:
            ...
            if href in oeb.manifest.hrefs:
                ...
                if hasattr(item.data, "xpath") and base.XPath("//h:a[@href]")(
                    item.data
                ):
        ...
        body = base.element(
            contents, base.tag("xhtml", "body"), attrib={"class": "calibre_toc"}
        )
        h1 = base.element(
            body, base.tag("xhtml", "h2"), attrib={"class": "calibre_toc_header"}
        )
        h1.text = title
    ...
    def add_toc_level(self, elem, toc):
        for node in toc:
            block = base.element(
                elem, base.tag("xhtml", "div"), attrib={"class": "calibre_toc_block"}
            )
            line = base.element(
                block,
                base.tag("xhtml", "a"),
                attrib={"href": node.href, "class": "calibre_toc_line"},
            )
