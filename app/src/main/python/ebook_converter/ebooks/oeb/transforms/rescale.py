...
class RescaleImages(object):
    ...
    def rescale(self):
        ...
        if is_image_collection:
            ...
            page_width -= (
                (self.opts.margin_left + self.opts.margin_right)
                * self.opts.dest.dpi
                / 72
            )
            page_height -= (
                (self.opts.margin_top + self.opts.margin_bottom)
                * self.opts.dest.dpi
                / 72
            )
        ...
        for item in self.oeb.manifest:
            if item.media_type.startswith("image"):
                ...
                scaled, new_width, new_height = uimg.fit_image(
                ...
                if scaled:
                    ...
                    self.log.info(
                        "Rescaling image from %dx%d to %dx%d %s",
                        width,
                        height,
                        new_width,
                        new_height,
