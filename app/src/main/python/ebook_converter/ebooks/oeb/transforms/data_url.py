        ...
        for item in oeb.spine:
            ...
            for img in attr_path(root):
                ...
                if not fmt:
                    ...
                    continue
                img.set(
                    "src", item.relhref(self.convert_image_data_uri(data, fmt, oeb))
                )

    def convert_image_data_uri(self, data, fmt, oeb):
        self.log.info("Found image encoded as data URI converting it to normal image")
        item_id, item_href = oeb.manifest.generate(
            "data-url-image", "data-url-image." + fmt
        )
        oeb.manifest.add(
            item_id, item_href, mimetypes.guess_type(item_href)[0], data=data
        )
        return item_href
