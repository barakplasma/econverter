...
class SubsetFonts(object):
    ...
    def __call__(self, oeb, log, opts):
        ...
        for font in fonts.values():
            ...
            try:
                ...
                self.log.warning(
                    "The font %s is unsupported for subsetting. %s", font["src"], e
                )
        ...
        if totals[0]:
            self.log(
                "Reduced total font size to %.1f%% of original",
    ...
    def used_font(self, style):
        ...
        if fw == 400:
            ...
            q = (
                [fw]
                + list(range(fw - 100, -100, -100))
                + list(range(fw + 100, 100, 1000))
            )
        else:
            q = (
                [fw]
    ...
    def find_usage_in(self, elem, inherited_style):
        style = elem_style(
            self.style_rules, elem.get("class", "") or "", inherited_style
        )
        for child in elem:
