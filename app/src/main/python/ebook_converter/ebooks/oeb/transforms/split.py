...
class SplitError(ValueError):
    def __init__(self, path, root):
        ...
        ValueError.__init__(
            self,
            "Could not find reasonable point at which "
...
class Split(object):
    ...
    def __call__(self, oeb, opts):
        self.oeb = oeb
        self.log = oeb.log
        self.log.info("Splitting markup on page breaks and flow limits, if any...")
        self.opts = opts
        self.map = {}
    ...
    def split_item(self, item):
        ...
        splitter = FlowSplitter(
            item, page_breaks, page_break_ids, self.max_flow_size, self.oeb, self.opts
        )
    ...
    def find_page_breaks(self, item):
        if self.page_break_selectors is None:
            ...
            stylesheets = [
                x.data for x in self.oeb.manifest if x.media_type in base.OEB_STYLES
            ]
        ...
        for selector, before in self.page_break_selectors:
            try:
                for elem in select(selector):
                    if (
                        elem in descendants
                        and elem.tag.rpartition("}")[2].lower() not in _tags
                    ):
...
class FlowSplitter(object):
    ...
    def split_on_page_breaks(self, orig_tree):
        ...
        while ordered_ids:
            ...
            for i in range(len(self.trees) - 1, -1, -1):
                ...
                if elem:
                    self.log.debug(
                        "\t\tSplitting on page-break at id=%s", elem[0].get("id")
                    )
                    ...
                    if body is not None:
                        ...
                        for x in ids - existing_ids:
                            body.insert(
                                0,
                                body.makeelement(
                                    base.tag("xhtml", "div"), id=x, style="height:0pt"
                                ),
                            )
                ids = set()
                trees.append(tree)
    ...
    def is_page_empty(self, root):
        ...
        if body is None:
            return False
        txt = re.sub(
            r"\s+|\xa0", "", etree.tostring(body, method="text", encoding="unicode")
        )
        if len(txt) > 1:
            return False
    ...
    def split_to_size(self, tree):
        ...
        self.log.debug(
            "\t\t\tSplit point: %s %s", split_point.tag, tree.getpath(split_point)
        )
        ...
        for t, size in zip(trees, sizes):
            ...
            if self.is_page_empty(r):
                ...
                self.log.debug(
                    ...
                    len(self.split_trees),
                    size / 1024.0,
                )
            else:
                self.log.debug("\t\t\tSplit tree still too large: %d KB", size / 1024)
    ...
    def commit(self):
        ...
        for current, tree in zip(*map(reversed, (self.files, self.trees))):
            ...
            new_item = self.oeb.manifest.add(
                new_id, current, self.item.media_type, data=tree.getroot()
            )
