...
def html5_parse(data, max_nesting_depth=100):
    ...
    data = parse(
        clean_xml_chars(data), maybe_xhtml=True, keep_doctype=False, sanitize_names=True
    )
    ...
    for x in data.iterdescendants():
        if isinstance(x.tag, (str, bytes)) and not len(x):  # Leaf node
            ...
            if depth > max_nesting_depth:
                raise ValueError(
                    "HTML 5 parsing resulted in a tree with nesting"
...
def clean_word_doc(data, log):
    ...
    if prefixes:
        ...
        pat = re.compile(
            r"<(%s):([a-zA-Z0-9]+)[^>/]*?></\1:\2>" % ("|".join(prefixes)), re.DOTALL
        )
        data = pat.sub("", data)
...
def check_for_html5(prefix, root):
    if re.search(r"<!DOCTYPE\s+html\s*>", prefix, re.IGNORECASE) is not None:
        if root.xpath("//svg"):
            raise HTML5Doc(
                "This document appears to be un-namespaced HTML 5, should be parsed by the HTML 5 parser"
            )
...
def parse_html(
    ...
    if idx > -1:
        ...
        if "<!DOCTYPE" in pre:  # Handle user defined entities
            ...
            has_html4_doctype = (
                re.search(r"<!DOCTYPE\s+[^>]+HTML\s+4.0[^.]+>", pre) is not None
            )
    ...
    if (
        has_html4_doctype
        or data.tag == "HTML"
        or (len(data) and (data[-1].get("LANG") or data[-1].get("DIR")))
    ):
    ...
    if not namespace(data.tag):
        ...
        nroot = etree.Element(
            XHTML("html"), nsmap={None: const.XHTML_NS}, attrib=attrib
        )
        for elem in data.iterdescendants():
            if isinstance(elem.tag, (str, bytes)) and namespace(elem.tag) == ns:
    ...
    )  # Ensure content is second attribute
    ...
    for a in xpath(data, "//h:a[@href]|//h:i|//h:b|//h:u"):
        if (
            a.get("id", None) is None
            and a.get("name", None) is None
            and len(a) == 0
            and not a.text
        ):
            remove_elem(a)
