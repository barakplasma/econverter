...
def simple_normalizer(prefix, names, check_inherit=True):
    ...
    def wrapper(name, cssvalue):
        return normalize_simple_composition(
            name, cssvalue, composition, check_inherit=check_inherit
        )
    ...
    class TestNormalization(unittest.TestCase):
        ...
        def test_border_normalization(self):
            ...
            def border_val_dict(expected, val="color"):
                ans = {
                    "border-%s-%s" % (edge, val): DEFAULTS["border-%s-%s" % (edge, val)]
                    for edge in EDGES
                }
                for edge in EDGES:
            ...
            for raw, expected in {
                ...
                for edge in EDGES:
                    br = "border-%s" % edge
                    val = tuple(parseStyle("%s: %s" % (br, raw), validate=False))[
                        0
                    ].cssValue
                    self.assertDictEqual(
                        border_edge_dict(expected, edge), normalizers[br](br, val)
                    )
            ...
            for raw, expected in {
                ...
                val = tuple(parseStyle("%s: %s" % ("border", raw), validate=False))[
                    0
                ].cssValue
                self.assertDictEqual(
                    border_dict(expected), normalizers["border"]("border", val)
                )
            ...
            for name, val in {
                ...
                cval = tuple(parseStyle("border-%s: %s" % (name, val), validate=False))[
                    0
                ].cssValue
                self.assertDictEqual(
                    border_val_dict(val, name),
                    normalizers["border-" + name]("border-" + name, cval),
                )
        ...
        def test_edge_normalization(self):
            ...
            for raw, expected in {
                ...
                for prefix in ("margin", "padding"):
                    cval = tuple(parseStyle("%s: %s" % (prefix, raw), validate=False))[
                        0
                    ].cssValue
                    self.assertDictEqual(
                        edge_dict(prefix, expected), normalizers[prefix](prefix, cval)
                    )
        ...
        def test_list_style_normalization(self):
            ...
            for raw, expected in {
                ...
                cval = tuple(parseStyle("list-style: %s" % raw, validate=False))[
                    0
                ].cssValue
                self.assertDictEqual(
                    ls_dict(expected), normalizers["list-style"]("list-style", cval)
                )
