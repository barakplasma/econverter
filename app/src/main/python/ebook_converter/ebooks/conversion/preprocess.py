...
convert_entities = functools.partial(
    entities.entity_to_unicode,
    result_exceptions={
        "<": "&lt;",
        ">": "&gt;",
        "'": "&apos;",
...
class Dehyphenator(object):
    ...
    def __init__(self, verbose=0, log=None):
        ...
        # hyphenation
        self.suffix_string = (
            "((ed)?ly|'?e?s||a?(t|s)?ion(s|al(ly)?)?|ings?"
    ...
    def dehyphenate(self, match):
        ...
        if self.verbose > 2:
            self.log.info("lookup word is: %s, orig is: %s", lookupword, hyphenated)
        try:
        ...
        if self.format == "html_cleanup" or self.format == "txt_cleanup":
            if self.html.find(lookupword) != -1 or searchresult != -1:
                if self.verbose > 2:
                    self.log.info(
                        "    Cleanup:returned dehyphenated word: %s", dehyphenated
                    )
                return dehyphenated
            elif self.html.find(hyphenated) != -1:
                if self.verbose > 2:
                    self.log.info(
                        "        Cleanup:returned hyphenated word: %s", hyphenated
                    )
                return hyphenated
            else:
                if self.verbose > 2:
                    self.log.info(
                        "            Cleanup:returning original text %s + linefeed %s",
                        firsthalf,
                        secondhalf,
                    )
            ...
            if (
                self.format == "individual_words"
                and len(firsthalf) + len(secondhalf) <= 6
            ):
                if self.verbose > 2:
                    self.log.info("too short, returned hyphenated word: %s", hyphenated)
                return hyphenated
            ...
            if self.html.find(lookupword) != -1 or searchresult != -1:
                if self.verbose > 2:
                    self.log.info("     returned dehyphenated word: ", dehyphenated)
                return dehyphenated
            else:
                if self.verbose > 2:
                    self.log.info("          returned hyphenated word: ", hyphenated)
                return hyphenated
    ...
    def __call__(self, html, format, length=1):
        ...
        if format == "html":
            ...
            intextmatch = re.compile(
                ...
                r"<[iub]>)\s*(?P<secondpart>[\w\d]+)" % length
            )
        elif format == "txt":
            intextmatch = re.compile(
                "(?<=.{%i})(?P<firstpart>[^\\W\\-]+)(-|‐)"
...
class CSSPreProcessor(object):
    ...
    def __call__(self, data, add_namespace=False):
        ...
        for line in data.splitlines():
            ...
            if not (
                namespaced
                or ll.startswith("@import")
                or not ll
                or ll.startswith("@charset")
            ):
...
def accent_regex(accent_maps, letter_before=False):
    ...
    pat = re.compile(
        r"([{}])\s*(?:<br[^>]*>){{0,1}}\s*([{}])".format(*args), re.UNICODE
    )
...
class HTMLPreProcessor(object):
    ...
    def is_baen(self, src):
        return (
            re.compile(
                r'<meta\s+name="Publisher"\s+content=".*?Baen.*?"', re.IGNORECASE
            ).search(src)
            is not None
        )
    ...
    def __call__(self, html, remove_special_chars=None, get_preprocess_html=False):
        ...
        def do_search_replace(search_pattern, replace_txt):
            ...
            try:
                ...
                self.log.error("Failed to parse %r regexp because %s", search, e)
            ...
            if length:
                ...
                end_rules.append(
                    ...
                    (
                        re.compile(
                            (
                                r"(?<=.{%i}([a-zäëïöüàèìòùáćéíĺóŕńśúýâêîôûçą"
        ...
        for rule in rules + end_rules:
            try:
                ...
                if rule in user_sr_rules:
                    self.log.error(
                        "User supplied search & replace rule: %s "
        ...
        try:
            unsupported_unicode_chars = (
                self.extra_opts.output_profile.unsupported_unicode_chars
            )
