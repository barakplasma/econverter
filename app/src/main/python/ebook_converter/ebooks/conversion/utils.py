...
class HeuristicProcessor(object):
    def __init__(self, extra_opts=None, log=None):
        ...
        self.blankreg = re.compile(
            r"\s*(?P<openline><p(?!\sclass=\"(softbreak|whitespace)\")[^>]*>)\s*(?P<closeline></p>)",
            re.IGNORECASE,
        )
        self.anyblank = re.compile(
            r"\s*(?P<openline><p[^>]*>)\s*(?P<closeline></p>)", re.IGNORECASE
        )
        self.multi_blank = re.compile(
            r"(\s*<p[^>]*>\s*</p>(\s*<div[^>]*>\s*</div>\s*)*){2,}(?!\s*<h\d)",
            re.IGNORECASE,
        )
        self.any_multi_blank = re.compile(
            r"(\s*<p[^>]*>\s*</p>(\s*<div[^>]*>\s*</div>\s*)*){2,}", re.IGNORECASE
        )
        self.line_open = (
        ...
        self.single_blank = re.compile(
            r"(\s*<(p|div)[^>]*>\s*</(p|div)>)", re.IGNORECASE
        )
    ...
    def chapter_head(self, match):
        ...
        if not title:
            ...
            self.log.debug(
                "marked %s chapters. - %s", self.html_preprocess_sections, str(chap)
            )
            return "<h2>" + chap + "</h2>\n"
        else:
            delete_whitespace = re.compile("^\\s*(?P<c>.*?)\\s*$")
            delete_quotes = re.compile("'\"")
            txt_chap = delete_quotes.sub(
                "", delete_whitespace.sub("\\g<c>", html2text(chap))
            )
            txt_title = delete_quotes.sub(
                "", delete_whitespace.sub("\\g<c>", html2text(title))
            )
            self.html_preprocess_sections = self.html_preprocess_sections + 1
            self.log.debug(
                "marked %s chapters & titles. - %s, %s",
    ...
    def chapter_break(self, match):
        ...
        self.html_preprocess_sections = self.html_preprocess_sections + 1
        self.log.debug(
            "marked %s section markers based on punctuation. - %s",
            self.html_preprocess_sections,
            chap,
        )
    ...
    def markup_italicis(self, html):
        ...
        for word in ITALICIZE_WORDS:
            html = re.sub(
                r"(?<=\s|>)" + re.escape(word) + r"(?=\s|<)", "<i>%s</i>" % word, html
            )
        ...
        for pat in ITALICIZE_STYLE_PATS:
            for match in re.finditer(pat, search_text):
                ...
                try:
                    html = re.sub(
                        re.escape(str(match.group(0))), "<i>%s</i>" % ital_string, html
                    )
    ...
    def markup_chapters(self, html, wordcount, blanks_between_paragraphs):
        ...
        self.log.debug("found %s pre-existing headings", self.html_preprocess_sections)
        ...
        simple_title = (
            r"(<[ibu][^>]*>)?\s{0,3}(?!(Chapter|\s+<)).{0,65}?(</[ibu][^>]*>)?(?=<)"
        )
                ...
                if analyze:
                    ...
                    if hits:
                        ...
                        if float(self.chapters_with_title) / float(hits) > 0.5:
                            ...
                            strict_title = False
                        self.log.debug(
                            "%s had %s hits %s chapters with no "
                        ...
                        if type_name == "common":
                            ...
                            self.min_chapters <= hits < max_chapters
                            or self.min_chapters < 3 > hits
                        ):
                            analysis_result.append(
                                [
                                    chapter_type,
        ...
        self.log.debug(
            "Total wordcount is: %s, Average words per section "
            "is: %s, Marked up %s chapters",
            wordcount,
            words_per_chptr,
            self.html_preprocess_sections,
        )
    ...
    def punctuation_unwrap(self, length, content, format):
        ...
        lookahead = (
            "(?<=.{"
            + str(length)
            + r"}([a-zა-ჰäëïöüàèìòùáćéíĺóŕńśúýâêîôûçąężıãõñæøþðßěľščťžňďřů,:)\\IAß]|(?<!\&\w{4});))"
        )
        ...
        if format == "txt":
            ...
            em_en_unwrap_regex = (
                em_en_lookahead + line_ending + blanklines + line_opening
            )
    ...
    def markup_pre(self, html):
        ...
        if len(pre.findall(html)) >= 1:
            ...
            outerhtml = re.compile(
                r".*?(?<=<pre>)(?P<text>.*?)</pre>", re.IGNORECASE | re.DOTALL
            )
    ...
    def fix_nbsp_indents(self, html):
        txtindent = re.compile(
            str(
                r"<(?P<tagtype>p|div)(?P<formatting>[^>]*)>\s*(?P<span>(<span[^>]*>\s*)+)?\s*(\u00a0){2,}"
            ),
            re.IGNORECASE,
        )
        ...
        if self.found_indents > 1:
            self.log.debug(
                "replaced %s nbsp indents with inline styles", self.found_indents
            )
        return html
    ...
    def cleanup_markup(self, html):
        ...
        open_fmt_pat, close_fmt_pat = (
            r"<(?:{})(?:\s[^>]*)?>".format(fmt_tags),
            "</(?:{})>".format(fmt_tags),
        )
        for i in range(2):
            html = re.sub(
                r"\s*<span[^>]*>\s*(<span[^>]*>\s*</span>){0,2}\s*</span>\s*", " ", html
            )
            html = re.sub(
                r"\s*{open}\s*({open}\s*{close}\s*){{0,2}}\s*{close}".format(
                    open=open_fmt_pat, close=close_fmt_pat
                ),
                " ",
                html,
            )
    ...
    def analyze_blanks(self, html):
        ...
        if len(lines) > 1:
            self.log.debug(
                "There are %s blank lines. %s percent blank",
                len(blanklines),
                len(blanklines) / len(lines),
            )
    ...
    def detect_whitespace(self, html):
        ...
        def merge_header_whitespace(match):
            ...
            if initblanks is not None:
                top_margin = (
                    "margin-top:"
                    + str(len(self.single_blank.findall(initblanks)))
                    + "em;"
                )
            if endblanks is not None:
                bottom_margin = (
                    "margin-bottom:"
                    + str(len(self.single_blank.findall(endblanks)))
                    + "em;"
                )
    ...
    def detect_soft_breaks(self, html):
        ...
        line_two = (
            "(?P<line_two>"
            + re.sub("(ou|in|cha)", "linetwo_", self.line_open)
            + "\\s*(?P<line_two_content>.*?)"
            + re.sub("(ou|in|cha)", "linetwo_", self.line_close)
            + ")"
        )
        div_break_candidate_pattern = line + "\\s*<div[^>]*>\\s*</div>\\s*" + line_two
        div_break_candidate = re.compile(
            r"%s" % div_break_candidate_pattern, re.IGNORECASE | re.UNICODE
        )

        def convert_div_softbreaks(match):
            init_is_paragraph = self.check_paragraph(match.group("init_content"))
            line_two_is_paragraph = self.check_paragraph(
                match.group("line_two_content")
            )
        ...
        if not self.blanks_deleted and self.blanks_between_paragraphs:
            html = self.multi_blank.sub(
                '\n<p class="softbreak" style="margin-top:1em; page-break-before:avoid; text-align:center"> </p>',
                html,
            )
        else:
    ...
    def detect_scene_breaks(self, html):
        scene_break_regex = (
            ...
            + "<))(?P<break>((?P<break_char>((?!\\s)\\W))\\s*(?P=break_char)?)+)\\s*"
            + self.line_close
        )
            ...
            if re.match("^<hr", replacement_break):
                if replacement_break.find("width") != -1:
                    try:
                        ...
                        scene_break = (
                            hr_open
                            + '<hr style="height: 3px; background:#505050" /></div>'
                        )
                        self.log.warning(
                            "Invalid replacement scene break expression, using default"
                        )
                    else:
                    ...
                    scene_break = (
                        hr_open + '<hr style="height: 3px; background:#505050" /></div>'
                    )
    ...
    def abbyy_processor(self, html):
        abbyy_line = re.compile(
            '((?P<linestart><p\\sstyle="(?P<styles>[^"]*?);?">)(?P<content>.*?)(?P<lineend></p>)|(?P<image><img[^>]*>))',
            re.IGNORECASE,
        )
        empty_paragraph = "\n<p> </p>\n"
            ...
            if image:
                ...
                if debugabby:
                    ...
                    self.log.debug(
                        ...
                        + blockquote_close_loop
                        + paragraph_before
                        + '<p style="%s">%s</p>' % (text_indent + text_align, content)
                        + paragraph_after
                        + "\n\n\n\n\n",
                    )
    ...
    def __call__(self, html):
        ...
        if getattr(self.extra_opts, "markup_chapter_headings", False):
            html = self.markup_chapters(
                html, self.totalwords, self.blanks_between_paragraphs
            )
        ...
        self.log.debug(
            "Median line length is %s, calculated with %s format", length, format
        )
        ...
        if self.html_preprocess_sections < self.min_chapters and getattr(
            ...
            self.log.debug(
                "Looking for more split points based on punctuation, currently have %s",
                self.html_preprocess_sections,
            )
        ...
        if getattr(self.extra_opts, "format_scene_breaks", False):
            ...
            if replacement_break:
                ...
                if scene_break_count >= 1:
                    ...
                    html = re.sub(
                        '<p\\s+class="softbreak"[^>]*>\\s*</p>', replacement_break, html
                    )
                else:
                    html = re.sub(
                        '<p\\s+class="softbreak"[^>]*>\\s*</p>', replacement_break, html
