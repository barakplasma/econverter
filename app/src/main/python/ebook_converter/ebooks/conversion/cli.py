...
def check_command_line_options(parser, args, log):
    ...
    if (
        not input_file.endswith(".recipe")
        and not os.access(input_file, os.R_OK)
        and not ("-h" in args or "--help" in args)
    ):
    ...
    if (
        ...
        and output_file[:2] not in {"..", "."}
        and "/" not in output_file
        and "\\" not in output_file
    ):
...
def option_recommendation_to_cli_option(add_option, rec):
    ...
    attrs = dict(
        dest=opt.name, help=opt.help, choices=opt.choices, default=rec.recommended_value
    )
    ...
    if opt.name == "transform_css_rules":
        attrs["help"] = (
            "Path to a file containing rules to transform the "
...
def add_input_output_options(parser, plumber):
    ...
    def add_options(group, options):
        for opt in options:
            if plumber.input_fmt == "recipe" and opt.option.long_switch == "test":
                group(
                    optparse.Option(
                        "--test", dest="test", action="callback", callback=recipe_test
                    )
                )
            else:
    ...
    if input_options:
        ...
        io = optparse.OptionGroup(
            parser,
            title,
            "Options to control the "
            "processing of the input %s file" % plumber.input_fmt,
        )
    ...
    if output_options:
        ...
        oo = optparse.OptionGroup(
            parser,
            title,
            "Options to control the processing of the output %s" % plumber.output_fmt,
        )
    ...
    groups = collections.OrderedDict(
        (
            ...
            (
                ...
                (
                    "Modify the document text and structure "
...
def create_option_parser(args, log):
    if "--version" in args:
        ...
        log.info("%s (%s, %s)", os.path.basename(args[0]), __appname__, __version__)
    ...
    for line in lines:
        if pat is None:
            ...
            try:
                ...
                msg = "Invalid regular expression: %r from file: %r" % (line, path)
                if log is not None:
                    log.error(msg)
                    raise SystemExit(1)
                else:
...
def main(args=sys.argv):
    ...
    if len(leftover_args) > 3:
        log.error("Extra arguments not understood: %s", ", ".join(leftover_args[3:]))
        return 1
    ...
    recommendations = [
        (n.dest, getattr(opts, n.dest), OptionRecommendation.HIGH)
