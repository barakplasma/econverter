...
class Plumber(object):
    ...
    def __init__(
        ...
        self.pipeline_options = [
            ...
            OptionRecommendation(
                ...
                "Choices are:" + ", ".join([x.short_name for x in output_profiles()]),
            ),
            OptionRecommendation(
                name="base_font_size",
                recommended_value=0,
            ...
            OptionRecommendation(
                ...
                recommended_value=False,
                level=OptionRecommendation.LOW,
                help="Don't add auto-detected chapters to the Table of Contents.",
            ),
            OptionRecommendation(
                name="toc_threshold",
                recommended_value=6,
            ...
            if val is not None:
                if x == "authors":
                    ...
                    try:
                        ...
                        self.log.warning(
                            "Values of series index and rating "
                            "must be numbers. Ignoring %s",
                            val,
                        )
                        continue
    ...
    def read_user_metadata(self):
        ...
        if mi.cover:
            if mi.cover.startswith("http:") or mi.cover.startswith("https:"):
                self.log.warning(
                    "TODO: Cover image is on remote server, "
    ...
    def setup_options(self):
        ...
        self.opts.no_inline_navbars = (
            self.opts.output_profile.supports_mobi_indexing
        ...
        if self.changed_options:
            ...
            for rec in self.changed_options:
                if rec.option.name not in ("username", "password"):
                    self.log.info(
                        " %s: %s", rec.option.name, repr(rec.recommended_value)
                    )
        if self.opts.verbose > 1:
            self.log.debug("Resolved conversion options")
            try:
                self.log.debug("ebook_converter version: %s", constants.VERSION)
    ...
    def dump_input(self, ret, output_dir):
        ...
        if self.input_fmt == "recipe":
            zf = ZipFile(
                os.path.join(self.opts.debug_pipeline, "periodical.downloaded_recipe"),
                "w",
            )
            zf.add_dir(out_dir)
    ...
    def run(self):
        ...
        if self.input_fmt == "azw4" and self.output_plugin.file_type == "pdf":
            self.ui_reporter(
                0.01,
                "AZW4 files are simply wrappers around PDF files."
            ...
            self.log.info(
                "%s output written to %s", self.output_fmt.upper(), self.output
            )
            self.flush()
            return
        ...
        with self.input_plugin:
            self.oeb = self.input_plugin(
                stream, self.opts, self.input_fmt, self.log, accelerators, tdir
            )
            if self.opts.debug_pipeline is not None:
            ...
            if not hasattr(self.oeb, "manifest"):
                self.oeb = create_oebbook(
                    ...
                    removed_items=getattr(
                        self.input_plugin, "removed_items_to_ignore", ()
                    ),
                )
            if self.for_regex_wizard:
                return
        ...
        pr(0.0, "Running transforms on e-book...")
        ...
        if self.opts.linearize_tables and self.output_plugin.file_type not in (
            "mobi",
            "lrf",
        ):
            from ebook_converter.ebooks.oeb.transforms.linearize_tables import (
                LinearizeTables,
            )
        ...
        if self.opts.unsmarten_punctuation:
            from ebook_converter.ebooks.oeb.transforms.unsmarten import (
        ...
        flattener = CSSFlattener(
            ...
            specializer=functools.partial(
                self.output_plugin.specialize_css_for_output, self.log, self.opts
            ),
        )
        ...
        our(0.0, "Running %s plugin" % self.output_plugin.name)
        with self.output_plugin:
            self.output_plugin.convert(
                self.oeb, self.output, self.input_plugin, self.opts, self.log
            )
        self.oeb.clean_temp_files()
        self.ui_reporter(1.0)
        run_plugins_on_postprocess(self.output, self.output_fmt)

        self.log.info("%s output written to %s", self.output_fmt.upper(), self.output)
        self.flush()
...
def create_oebbook(
    ...
    html_preprocessor = HTMLPreProcessor(
        log, opts, regex_wizard_callback=regex_wizard_callback
    )
    if not encoding:
        encoding = None
    oeb = OEBBook(
        log, html_preprocessor, pretty_print=opts.pretty_print, input_encoding=encoding
    )
    if not populate:
        return oeb
