...
class RTFInput(InputFormatPlugin):
    ...
    def replace_wmf(self, name):
        ...
        if self.default_img is None:
            self.default_img = message_image(
                "Conversion of WMF images is not "
    ...
    def write_inline_css(self, ic, border_styles):
        ...
        color_classes = [
            ...
            for i, x in enumerate(ic.colors)
            if x != "false"
        ]
        css = textwrap.dedent("""
        span.none {
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        ...
        for pict in doc.xpath(
            "//rtf:pict[@num]", namespaces={"rtf": "http://rtf2xml.sourceforge.net/"}
        ):
        ...
        styledoc = etree.fromstring(
            (importlib.resources.files("ebook_converter") / "data/rtf.xsl").read_bytes()
        )
