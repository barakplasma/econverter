...
class ComicInput(InputFormatPlugin):
    ...
    options = {
        ...
        OptionRecommendation(
            ...
            recommended_value=False,
            help="Maintain picture aspect ratio. Default is to fill the screen.",
        ),
        OptionRecommendation(
            name="dont_sharpen", recommended_value=False, help="Disable sharpening."
        ),
        OptionRecommendation(
            name="disable_trim",
            recommended_value=False,
    ...
    def get_pages(self, comic, tdir2):
        from ebook_converter.ebooks.comic.input import (
            extract_comic,
            process_pages,
            find_pages,
        )
        ...
        new_pages = find_pages(
            tdir, sort_on_mtime=self.opts.no_sort, verbose=self.opts.verbose
        )
        thumbnail = None
        if not new_pages:
            raise ValueError("Could not find any pages in the comic: %s" % comic)
        if self.opts.no_process:
            n2 = []
            for i, page in enumerate(new_pages):
                n2.append(
                    os.path.join(tdir2, "{} - {}".format(i, os.path.basename(page)))
                )
                shutil.copyfile(page, n2[-1])
            new_pages = n2
        else:
            new_pages, failures = process_pages(
                new_pages, self.opts, self.report_progress, tdir2
            )
            if failures:
                self.log.warning(
                    "Could not process the following pages "
                    "(run with --verbose to see why):"
                )
                for f in failures:
                    self.log.warning("\t%s", f)
            if not new_pages:
                raise ValueError("Could not find any valid pages in comic: %s" % comic)
        ...
        if len(comics) == 1:
            ...
            for comic in comics:
                ...
                if not opts.dont_add_comic_pages_to_toc:
                    for i, x in enumerate(wrappers):
                        stoc.add_item(href(x), None, "Page %d" % (i + 1), play_order=po)
                        po += 1
        opf.set_toc(toc)
    ...
    def create_wrappers(self, pages):
        ...
        for i, page in enumerate(pages):
            wrapper = WRAPPER % (const.XHTML_NS, i + 1, os.path.basename(page), i + 1)
