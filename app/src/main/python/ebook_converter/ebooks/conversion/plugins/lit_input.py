        ...
        for item in oeb.spine:
            ...
            if body:
                ...
                if len(body) == 1 and body[0].tag == XHTML("pre"):
                    ...
                    self.log.info("LIT file with all text in singe <pre> tag detected")
                    html = separate_paragraphs_single_line(pre.text)
                    html = convert_basic(html).replace(
                        "<html>", '<html xmlns="%s">' % const.XHTML_NS
                    )
                    html = xml_to_unicode(
                        html, strip_encoding_pats=True, resolve_entities=True
                    )[0]
                    if opts.smarten_punctuation:
                        # SmartyPants skips text inside <pre> tags
                        from ebook_converter.ebooks.conversion import preprocess
