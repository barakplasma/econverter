...
class FB2Input(InputFormatPlugin):
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        ...
        raw = xml_to_unicode(
            raw, strip_encoding_pats=True, assume_utf8=True, resolve_entities=True
        )[0]
        try:
        ...
        for s in stylesheets:
            css += (
                etree.tostring(s, encoding="unicode", method="text", with_tail=False)
                + "\n\n"
            )
        if css:
            import css_parser
            ...
            parser = css_parser.CSSParser(
                fetcher=None, log=logging.getLogger("calibre.css")
            )
        ...
        ss = (importlib.resources.files("ebook_converter") / "data/fb2.xsl").read_text()
        ss = ss.replace("__FB_NS__", fb_ns)
        if options.no_inline_fb2_toc:
            log.info("Disabling generation of inline FB2 TOC")
            ss = re.compile(
                r"<!-- BUILD TOC -->.*<!-- END BUILD TOC -->", re.DOTALL
            ).sub("", ss)
        ...
        if mi.cover_data and mi.cover_data[1]:
            ...
            for img in doc.xpath("//f:coverpage/f:image", namespaces=NAMESPACES):
                href = img.get("{%s}href" % const.XLINK_NS, img.get("href", None))
                if href is not None:
                    if href.startswith("#"):
                        href = href[1:]
        ...
        entries = [(f2, mimetypes.guess_type(f2)[0]) for f2 in os.listdir(".")]
        ...
        for elem in doc.xpath("./*"):
            if elem.text and "binary" in elem.tag and "id" in elem.attrib:
                ...
                try:
                    ...
                    self.log.exception(
                        "Binary data with id=%s is corrupted, ignoring", elem.get("id")
                    )
                else:
