...
class EPUBInput(InputFormatPlugin):
        ...
        try:
            ...
            for em in root.xpath('descendant::*[contains(name(), "EncryptionMethod")]'):
                ...
                path = os.path.abspath(
                    os.path.join(os.path.dirname(encfile), "..", *uri.split("/"))
                )
    ...
    def rationalize_cover2(self, opf, log):
        ...
        if not guide_cover:
            ...
            if raster_cover:
                ...
                guide_elem = g.makeelement(
                    base.tag("opf", "reference"),
                    attrib={"href": raster_cover, "type": "cover"},
                )
                g.append(guide_elem)
            return
        spine = list(opf.iterspine())
        ...
        if not self.for_viewer:
            if len(spine) == 1:
                log.warn(
                    "There is only a single spine item and it is marked "
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        ...
        try:
            ...
            log.exception(
                "EPUB appears to be invalid ZIP file, trying a "
        ...
        if opf is None:
            raise ValueError(
                "%s is not a valid EPUB file (could not find opf)" % _path_or_stream
            )
    ...
    def convert_epub3_nav(self, nav_path, opf, log, opts):
        ...
        ncx = etree.fromstring(
            '<ncx xmlns="http://www.daisy.org/z3986/2005/'
        ...
        def add_from_li(li, parent):
            ...
            if href:
                np.append(
                    np.makeelement(base.tag("ncx", "content"), attrib={"src": href})
                )
            return np
        ...
        with NamedTemporaryFile(
            suffix=".ncx", dir=os.path.dirname(nav_path), delete=False
        ) as f:
            f.write(etree.tostring(ncx, encoding="utf-8"))
        ncx_href = os.path.relpath(f.name, os.getcwd()).replace(os.sep, "/")
        ncx_id = opf.create_manifest_item(ncx_href, base.NCX_MIME, append=True).get(
            "id"
        )
        ...
        if getattr(self, "removed_cover", None):
            ...
            for elem in root.xpath("//*[@href]"):
                ...
                link_path = os.path.relpath(
                    os.path.join(base_path, urllib.parse.unquote(href)), base_path
                )
