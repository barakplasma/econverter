...
class PDFOutput(OutputFormatPlugin):
    ...
    options = {
        ...
        OptionRecommendation(
            ...
            help="The font family used to render serif fonts. Will work only if the font is available system-wide.",
        ),
        OptionRecommendation(
            name="pdf_sans_family",
            recommended_value="Helvetica",
            help="The font family used to render sans-serif fonts. Will work only if the font is available system-wide.",
        ),
        OptionRecommendation(
            name="pdf_mono_family",
            recommended_value="Courier",
            help="The font family used to render monospace fonts. Will work only if the font is available system-wide.",
        ),
        OptionRecommendation(
    ...
    def specialize_options(self, log, opts, input_fmt):
        ...
        if opts.pdf_use_document_margins:
            ...
            opts.margin_left = opts.margin_right = opts.margin_top = (
                opts.margin_bottom
            ) = -1
    ...
    def convert_images(self, images):
        ...
        convert(
            images, self.output_path, self.opts, self.metadata, self.report_progress
        )
            ...
            for href, margins in self.stored_page_margins.items():
                ...
                if item is not None:
                    ...
                    if hasattr(root, "xpath") and margins:
                        root.set(
                            "data-calibre-pdf-output-page-margins", json.dumps(margins)
                        )
