...
class SNBOutput(OutputFormatPlugin):
    ...
    def convert(self, oeb_book, output_path, input_plugin, opts, log):
        ...
        self.opts = opts
        from ebook_converter.ebooks.oeb.transforms.rasterize import (
            SVGRasterizer,
            Unavailable,
        )
        ...
        try:
            ...
            log.warning("SVG rasterizer unavailable, SVG will not be converted")
        ...
        with TemporaryDirectory("_snb_output") as tdir:
            ...
            etree.SubElement(headTree, "generator").text = (
            ...
            with open(os.path.join(snbfDir, "book.snbf"), "wb") as f:
                f.write(
                    etree.tostring(bookInfoTree, pretty_print=True, encoding="utf-8")
                )
            ...
            if oeb_book.toc.count() == 0:
                log.warning(
                    "This SNB file has no Table of Contents. Creating a default TOC"
                )
            ...
            with open(os.path.join(snbfDir, "toc.snbf"), "wb") as f:
                f.write(
                    etree.tostring(tocInfoTree, pretty_print=True, encoding="utf-8")
                )
                ...
                if m.hrefs[item.href].media_type in OEB_DOCS:
                    if item.href not in outputFiles:
                        ...
                        if oldTree is not None and mergeLast:
                            ...
                            with open(os.path.join(snbcDir, lastName), "wb") as f:
                                f.write(
                                    etree.tostring(
                                        oldTree, pretty_print=True, encoding="utf-8"
                                    )
                                )
                            mergeLast = False
                    ...
                    if not mergeLast:
                        ...
                        snbwriter.merge_content(
                            oldTree, oeb_book, item, [("", "Start")], opts
                        )
            ...
            if oldTree is not None and mergeLast:
                with open(os.path.join(snbcDir, lastName), "wb") as f:
                    f.write(
                        etree.tostring(oldTree, pretty_print=True, encoding="utf-8")
                    )
                mergeLast = False
            ...
            for item in m:
                if m.hrefs[item.href].media_type in OEB_IMAGES:
                    ...
                    self.HandleImage(
                        content, os.path.join(snbiDir, ProcessFileName(item.href))
                    )
    ...
    def HandleImage(self, imageData, imagePath):
        from ebook_converter.utils.img import (
            image_from_data,
            resize_image,
            image_to_data,
        )
...
if __name__ == "__main__":
    ...
    SNBOutput(None).convert(oeb, "/tmp/test.snb", None, None, logging.default_log)
