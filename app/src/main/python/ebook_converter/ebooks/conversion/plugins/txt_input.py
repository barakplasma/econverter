...
class TXTInput(InputFormatPlugin):
    ...
    options = {
        ...
        OptionRecommendation(
            ...
            + "\n".join(
                "* %s: %s" % (k, MD_EXTENSIONS[k]) for k in sorted(MD_EXTENSIONS)
            ),
        ),
    }
    ...
    def fix_resources(self, html, base_dir):
        ...
        for img in root.xpath("//img[@src]"):
            ...
            if prefix not in ("file", "http", "https", "ftp") and not os.path.isabs(
                src
            ):
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        ...
        if not ienc:
            ienc = "utf-8"
            log.debug(
                "No input encoding specified and could not auto detect using %s", ienc
            )
