...
class HTMLZOutput(OutputFormatPlugin):
    ...
    def convert(self, oeb_book, output_path, input_plugin, opts, log):
        ...
        if opts.htmlz_css_type == "inline":
            ...
            from ebook_converter.ebooks.htmlz.oeb2html import (
                OEB2HTMLClassCSSizer as OEB2HTMLizer,
            )
        ...
        with TemporaryDirectory("_htmlz_output") as tdir:
            ...
            if opts.htmlz_title_filename:
                ...
                fname = shorten_components_to(
                    100, (ascii_filename(str(oeb_book.metadata.title[0])),)
                )[0]
            ...
            with open(os.path.join(tdir, "metadata.opf"), "wb") as mdataf:
                opf = OPF(
                    io.BytesIO(
                        etree.tostring(oeb_book.metadata.to_opf1(), encoding="UTF-8")
                    )
                )
