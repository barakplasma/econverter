...
class LRFInput(InputFormatPlugin):
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        from ebook_converter.ebooks.lrf.input import (
            MediaType,
            Styles,
            TextBlock,
            Canvas,
        ...
        for x in doc.xpath("//CharButton[@refobj]"):
            ...
            if jump_button:
                jump_to = jump_button[0].xpath(
                    "descendant::JumpTo[@refpage and @refobj]"
                )
                if jump_to:
        ...
        for x in doc.xpath("//Plot[@refobj]"):
            ...
            if image:
                imgstr = doc.xpath(
                    '//ImageStream[@objid="%s" and @file]' % image[0].get("refstream")
                )
                if imgstr:
        ...
        styledoc = etree.fromstring(
            (importlib.resources.files("ebook_converter") / "data/lrf.xsl").read_bytes()
        )
