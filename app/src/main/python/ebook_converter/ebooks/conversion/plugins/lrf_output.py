    ...
    def __init__(self, output, opts, oeb):
        ...
        for x in ("top", "bottom", "left", "right"):
            setattr(
                self,
                x + "_margin",
                (self.profile.dpi / 72.0) * float(getattr(opts, "margin_" + x)),
            )
...
class LRFOutput(OutputFormatPlugin):
    ...
    options = {
        OptionRecommendation(
            ...
            help="Enable auto-rotation of images that are wider than the screen width.",
        ),
        OptionRecommendation(
            name="wordspace",
    ...
    def convert(self, oeb, output_path, input_plugin, opts, log):
        ...
        if input_plugin.is_image_collection:
            self.convert_images(
                input_plugin.get_images(), lrf_opts, getattr(opts, "wide", False)
            )
            return
