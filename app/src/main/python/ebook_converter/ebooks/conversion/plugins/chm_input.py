...
class CHMInput(InputFormatPlugin):
    ...
    def convert(self, stream, options, file_ext, log, accelerators):
        ...
        with TemporaryDirectory("_chm2oeb") as tdir:
            ...
            mainname = self._chmtohtml(
                tdir, chm_name, no_images, log, debug_dump=debug_dump
            )
            ...
            encoding = (
                self._chm_reader.get_encoding() or options.input_encoding or "cp1252"
            )
    ...
    def _create_html_root(self, hhcpath, log, encoding):
        ...
        hhcdata = xml_to_unicode(
            hhcdata, verbose=True, strip_encoding_pats=True, resolve_entities=True
        )[0]
        ...
        def unquote_path(x):
            y = unquote(x)
            if not os.path.exists(os.path.join(base, x)) and os.path.exists(
                os.path.join(base, y)
            ):
                x = y
            return x
        ...
        def donode(item, parent, base, subpath):
            for child in item:
                ...
                if not os.path.exists(os.path.join(base, rsrcpath)) and os.path.exists(
        ...
        with open(htmlpath, "wb") as f:
            if toc.count() > 1:
                ...
                raw = html.tostring(
                    builder.HTML(builder.BODY(root)),
                    encoding="utf-8",
                    pretty_print=True,
                )
                f.write(raw)
