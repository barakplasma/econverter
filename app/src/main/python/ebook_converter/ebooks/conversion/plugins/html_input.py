...
def sanitize_file_name(x):
    ans = (
        re.sub(r"\s+", " ", re.sub(r"[?&=;#]", "_", ascii_filename(x)))
        .strip()
        .rstrip(".")
    )
...
class HTMLInput(InputFormatPlugin):
    ...
    def create_oebbook(self, htmlpath, basedir, opts, log, mi):
        ...
        oeb = create_oebbook(
            log, None, opts, self, encoding=opts.input_encoding, populate=False
        )
        self.oeb = oeb
        ...
        for f in filelist:
            ...
            rewrite_links(item.data, functools.partial(self.resource_adder, base=dpath))
        ...
        for item in oeb.manifest.values():
            if item.media_type in self.OEB_STYLES:
                ...
                css_parser.replaceUrls(
                    item.data, functools.partial(self.resource_adder, base=dpath)
                )
        ...
        if link not in self.added_resources:
            ...
            if media_type == self.BINARY_MIME:
                ...
                try:
                    ...
                    if img:
                        media_type = (
                            mimetypes.guess_type("dummy." + img)[0] or self.BINARY_MIME
                        )

            self.oeb.log.debug("Added %s", link)
            self.oeb.container = self.DirContainer(
                os.path.dirname(link), self.oeb.log, ignore_opf=True
            )
            # Load into memory
