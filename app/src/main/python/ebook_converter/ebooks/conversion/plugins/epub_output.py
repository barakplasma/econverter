...
class EPUBOutput(OutputFormatPlugin):
    ...
    def convert(self, oeb, output_path, input_plugin, opts, log):
        ...
        if self.opts.epub_inline_toc:
            ...
            TOCAdder(
                oeb, opts, replace_previous_inline_toc=True, ignore_existing_toc=True
            )
        ...
        with TemporaryDirectory("_epub_output") as tdir:
            ...
            self.condense_ncx(
                [os.path.join(tdir, x) for x in os.listdir(tdir) if x.endswith(".ncx")][
                    0
                ]
            )
            ...
            with initialize_container(
                output_path, os.path.basename(opf), extra_entries=extra_entries
            ) as epub:
                epub.add_dir(tdir)
                if encryption is not None:
                    epub.writestr(
                        "META-INF/encryption.xml", polyglot.as_bytes(encryption)
                    )
    ...
    def upgrade_to_epub3(self, tdir, opf):
        ...
        from ebook_converter.ebooks.oeb.polish.cover import (
            fix_conversion_titlepage_links_in_nav,
        )
            ...
            for uri in list(uris.keys()):
                ...
                with open(path, "r+b") as f:
                    ...
                    if len(data) >= 1024:
                        ...
                        f.seek(0)
                        f.write(
                            bytes(bytearray(data[i] ^ key[i % 16] for i in range(1024)))
                        )
                    else:
    ...
    def workaround_ade_quirks(self):  # {{{
        ...
        for node in self.oeb.toc.iter():
            ...
            if hasattr(href, "partition"):
                ...
                if frag and frag_pat.match(frag) is None:
                    self.log.warning(
                        "Removing fragment identifier %r from "
        ...
        for x in self.oeb.spine:
            ...
            formchildren = base.XPath(
                "./h:input|./h:button|./h:textarea|./h:label|./h:fieldset|./h:legend"
            )
                    ...
                    for x in self.oeb.spine:
                        if x.href == href:
                            if frag_is_at_top(x.data, frag):
                                self.log.debug(
                                    "Removing anchor from TOC href: %s#%s", href, frag
                                )
                                toc.href = href
                            break
            for x in toc:
