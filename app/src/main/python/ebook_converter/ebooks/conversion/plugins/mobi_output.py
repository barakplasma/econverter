...
def remove_html_cover(oeb, log):
    ...
    if item.spine_position is not None:
        log.warning(
            "Found an HTML cover: %s",
            "removing it. If you find some "
...
class MOBIOutput(OutputFormatPlugin):
    ...
    def convert(self, oeb, output_path, input_plugin, opts, log):
        ...
        if create_kf8:
            from ebook_converter.ebooks.mobi.writer8.cleanup import (
                remove_duplicate_anchors,
            )
        ...
        kf8 = (
            self.create_kf8(resources, for_joint=mobi_type == "both")
            if create_kf8
            else None
        )
        if mobi_type == "new":
    ...
    def write_mobi(self, input_plugin, output_path, kf8, resources):
        ...
        try:
            ...
            self.log.warning("SVG rasterizer unavailable, SVG will not be converted")
        else:
