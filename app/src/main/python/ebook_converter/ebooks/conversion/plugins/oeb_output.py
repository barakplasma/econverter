...
class OEBOutput(OutputFormatPlugin):
    ...
    def workaround_nook_cover_bug(self, root):  # {{{
        cov = root.xpath(
            '//*[local-name() = "meta" and @name="cover" and @content != "cover"]'
        )
        ...
        if len(cov) == 1:
            ...
            if covid:
                ...
                if len(manifest_item) == 1 and manifest_item[0].get(
                    "media-type", ""
                ).startswith("image/"):
                    self.log.warning(
                        'The cover image has an id != "cover". '
    ...
    def workaround_pocketbook_cover_bug(self, root):  # {{{
        m = root.xpath(
            '//*[local-name() = "manifest"]/*[local-name() = "item"  and @id="cover"]'
        )
        if len(m) == 1:
            m = m[0]
            p = m.getparent()
