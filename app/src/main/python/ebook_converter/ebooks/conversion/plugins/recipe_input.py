        ...
        if file_ext == "downloaded_recipe":
            ...
            if os.environ.get("CALIBRE_RECIPE_URN"):
                ...
                raw = get_builtin_recipe_by_title(
                    title, log=log, download_recipe=not opts.dont_download_recipe
                )
                builtin = False
                try:
                ...
                if builtin:
                    ...
                    raw = get_builtin_recipe_by_title(
                        title, log=log, download_recipe=False
                    )
                    if raw is None:
            ...
            if recipe is None:
                raise ValueError(
                    "%r is not a valid recipe file or builtin recipe" % recipe_or_file
                )
    ...
    def specialize(self, oeb, opts, log, output_fmt):
        if opts.no_inline_navbars:
            ...
            for item in oeb.spine:
                for div in XPath('//h:div[contains(@class, "calibre_navbar")]')(
                    item.data
                ):
