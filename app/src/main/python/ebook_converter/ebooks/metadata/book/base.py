...
from ebook_converter.ebooks.metadata.book import (
    SC_COPYABLE_FIELDS,
...
def ck(typ):
    return typ.lower().strip().replace(":", "").replace(",", "")
...
class Metadata(object):
    ...
    def format_field_extended(self, key, series_with_index=True):
        ...
        if key in self.custom_field_keys():
            ...
            if (
                res is None or res == ""
            ):  # can't check "not res" because of numeric fields
        ...
        if fmkey in field_metadata and field_metadata[fmkey]["kind"] == "field":
            ...
            if key == "authors":
                ...
                res = fmeta["is_multiple"]["list_to_ui"].join(
                    sorted(filter(None, res), key=sort_key)
                )
            elif datatype == "series" and series_with_index:
                res = res + " [%s]" % self.format_series_index()
            elif datatype == "datetime":
    ...
    def __unicode__representation__(self):
        ...
        if self.authors:
            fmt(
                ...
                + (
                    (" [" + self.author_sort + "]")
                    if self.author_sort and self.author_sort != "Unknown"
                    else ""
                ),
            )
        if self.publisher:
        ...
        if self.rating is not None:
            fmt("Rating", ("%.2g" % (float(self.rating) / 2)) if self.rating else "")
        ...
        if self.identifiers:
            fmt(
                "Identifiers",
                ", ".join(["%s:%s" % (k, v) for k, v in self.identifiers.items()]),
            )
        if self.comments:
    ...
    def to_html(self):
        ...
        ans += [
            (
                "Author(s)",
                (authors_to_string(self.authors) if self.authors else "Unknown"),
            )
        ]
        ...
        if self.series:
            ans += [("Series", str(self.series) + " #%s" % self.format_series_index())]
        ans += [("Languages", ", ".join(self.languages))]
        if self.timestamp is not None:
            ans += [
                ("Timestamp", str(isoformat(self.timestamp, as_utc=False, sep=" ")))
            ]
        if self.pubdate is not None:
