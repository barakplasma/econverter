...
def get_metadata(stream):
    ...
    if book_title:
        ...
        book_title = uenc.force_unicode(
            os.path.splitext(os.path.basename(getattr(stream, "name", "Unknown")))[0]
        )
...
def _parse_cover(root, mi, ctx):
    ...
    imgid = ctx.XPath(
        'substring-after(string(//fb:coverpage/fb:image/@xlink:href), "#")'
    )(root)
    if imgid:
        try:
    ...
    if elm_binary:
        ...
        if not mime_extensions and mimetype.startswith("image/"):
            ...
            if mimetype_fromid and mimetype_fromid.startswith("image/"):
                mime_extensions = mimetypes.guess_all_extensions(mimetype_fromid)
        ...
        if mime_extensions:
            ...
            print(
                f"WARNING: Unsupported coverpage mime-type '{mimetype}' (id=#{imgid})"
            )
...
def _parse_series(root, mi, ctx):
    ...
    if elms_sequence:
        ...
        if mi.series:
            try:
                i = float(".".join(elms_sequence[0].get("number", None).split()[:2]))
                mi.series_index = i
            except Exception:
                pass
    ...
    if not mi.is_null("authors"):
        ...
        for author in reversed(mi.authors):
            ...
            if len(author_parts) == 1:
                ...
                if len(author_parts) > 1:
                    ctx.create_tag(
                        atag, "middle-name", at_start=False
                    ).text = author_parts[0]
                    author_parts = author_parts[1:]
                if author_parts:
                    a = " ".join(author_parts)
...
def _set_cover(title_info, mi, ctx):
    if not mi.is_null("cover_data") and mi.cover_data[1]:
        ...
        cim_binary = ctx.get_or_create(
            fb2_root, "binary", attribs={"id": cim_filename}, at_start=False
        )
