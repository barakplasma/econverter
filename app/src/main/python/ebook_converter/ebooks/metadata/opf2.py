...
from ebook_converter.constants_old import __appname__, __version__, filesystem_encoding
...
class MetadataField(object):
    def __init__(
        self, name, is_dc=True, formatter=None, none_is=None, renderer=lambda x: str(x)
    ):
        self.name = name
        self.is_dc = is_dc
...
class OPF(object):  # {{{
    ...
    authors_path = XPath(
        'descendant::*[re:match(name(), "creator", "i") '
        'and (@role="aut" or @opf:role="aut" or (not(@role) '
        "and not(@opf:role)))]"
    )
    bkp_path = XPath(
        'descendant::*[re:match(name(), "contributor", "i") and '
    ...
    raster_cover_path = XPath(
        'descendant::*[re:match(name(), "meta", "i") '
        'and re:match(@name, "cover", "i") and '
        "@content]"
    )
    guide_cover_path = XPath(
        'descendant::*[local-name()="guide"]/*[local-'
    ...
    manifest_path = XPath(
        'descendant::*[re:match(name(), "manifest", "i")]/'
        '*[re:match(name(), "item", "i")]'
    )
    manifest_ppath = XPath('descendant::*[re:match(name(), "manifest", "i")]')
    spine_path = XPath(
        'descendant::*[re:match(name(), "spine", "i")]/*[re:'
        'match(name(), "itemref", "i")]'
    )
    guide_path = XPath(
        'descendant::*[re:match(name(), "guide", "i")]/*[re:'
    ...
    if tweaks["use_series_auto_increment_tweak_when_importing"]:
        series_index = MetadataField(
            "series_index", is_dc=False, formatter=float, none_is=None
        )
    else:
        series_index = MetadataField(
            "series_index", is_dc=False, formatter=float, none_is=1
        )
    title_sort = TitleSortField("title_sort", is_dc=False)
    rating = MetadataField("rating", is_dc=False, formatter=float)
    publication_type = MetadataField("publication_type", is_dc=False)
    timestamp = MetadataField(
        "timestamp", is_dc=False, formatter=parse_date, renderer=isoformat
    )
    user_categories = MetadataField(
        "user_categories", is_dc=False, formatter=json.loads, renderer=dump_dict
    )
    author_link_map = MetadataField(
        "author_link_map", is_dc=False, formatter=json.loads, renderer=dump_dict
    )
    ...
    def __init__(
        ...
        if not self.metadata:
            self.metadata = [
                self.root.makeelement("{http://www.idpf.org/2007/opf}metadata")
            ]
    ...
    def read_user_metadata(self):
        ...
        elems = self.root.xpath(
            '//*[name() = "meta" and starts-with(@name,'
    ...
    def find_toc(self):
        self.toc = None
        try:
            spine = OPF.XPath('descendant::*[re:match(name(), "spine", "i")]')(
                self.root
            )
            toc = None
            if spine:
            ...
            if is_ncx or toc.lower() in ("ncx", "ncxtoc"):
                ...
                if not os.access(self.path_to_html_toc, os.R_OK) or not os.path.isfile(
                    self.path_to_html_toc
                ):
    ...
    def replace_spine_items_by_idref(self, idref, new_idrefs):
        ...
        spine = OPF.XPath('/opf:package/*[re:match(name(), "spine", "i")]')(self.root)[
            0
        ]
    ...
    def create_guide_item(self, type, title, href):
        e = etree.Element(
            "{%s}reference" % self.NAMESPACES["opf"], type=type, title=title, href=href
        )
        e.tail = "\n"
        return e
    ...
    def authors(self, val):
        ...
        for author in reversed(val):
            elem = self.metadata.makeelement(
                "{%s}creator" % self.NAMESPACES["dc"], nsmap=self.NAMESPACES
            )
            elem.tail = "\n"
    ...
    def pubdate(self):
        ...
        for match in self.pubdate_path(self.metadata):
            try:
                val = parse_date(
                    etree.tostring(
                        match, encoding="unicode", method="text", with_tail=False
                    ).strip()
                )
            except Exception:
                continue
    ...
    def get_identifiers(self):
        identifiers = {}
        for x in OPF.XPath('descendant::*[local-name() = "identifier" and text()]')(
            self.metadata
        ):
            found_scheme = False
            for attr, val in x.attrib.items():
                if attr.endswith("scheme"):
                    typ = val.lower()
                    val = etree.tostring(
                        x, with_tail=False, encoding="unicode", method="text"
                    ).strip()
    ...
    def set_identifiers(self, identifiers):
        ...
        for x in OPF.XPath('descendant::*[local-name() = "identifier"]')(self.metadata):
            xid = x.get("id", None)
            is_package_identifier = uuid_id is not None and uuid_id == xid
            typ = {
                val.lower() for attr, val in x.attrib.items() if attr.endswith("scheme")
            }
            if is_package_identifier:
                typ = tuple(typ)
        ...
        for typ, val in identifiers.items():
            ...
            self.set_text(
                self.create_metadata_element("identifier", attrib=attrib), str(val)
            )
    ...
    def page_progression_direction(self):
        spine = OPF.XPath('descendant::*[re:match(name(), "spine", "i")][1]')(self.root)
        if spine:
            for k, v in spine[0].attrib.items():
                if k == "page-progression-direction" or k.endswith(
    ...
    def primary_writing_mode(self):
        for m in OPF.XPath(
            '//*[local-name()="meta" and @name="primary-writing-mode" and @content]'
        )(self.root):
            ...
            for item in self.identifier_path(self.metadata):
                ...
                if item.text:
                    ...
                    for suffix in [".jpg", ".jpeg", ".gif", ".png", ".bmp"]:
                        cpath = os.access(
                            os.path.join(self.base_dir, prefix + suffix), os.R_OK
                        )
                        if os.access(
                            os.path.join(self.base_dir, prefix + suffix), os.R_OK
                        ):
                ...
                if href:
                    ...
                    if i is not None:
                        ...
                        if (
                            ...
                            and mt
                            and mt.lower()
                            in {"image/png", "image/jpeg", "image/jpg", "image/gif"}
                        ):
                            return i
    ...
    def render(self, encoding="utf-8"):
        ...
        # re-declarations
        smap = {}
        for child in self.metadata.xpath(
            './*[@name="calibre:series" or @name="calibre:series_index"]'
        ):
            smap[child.get("name")] = (child, self.metadata.index(child))
        if (
            len(smap) == 2
            and smap["calibre:series"][1] > smap["calibre:series_index"][1]
        ):
...
class OPFCreator(Metadata):
    ...
    def create_manifest(self, entries):
        ...
        entries = list(
            map(
                lambda x: (
                    x
                    if os.path.isabs(x[0])
                    else (os.path.abspath(os.path.join(self.base_path, x[0])), x[1])
                ),
                entries,
            )
        )
        ...
        entries = list(
            map(
                lambda x: (
                    x
                    if os.path.isabs(x)
                    else os.path.abspath(os.path.join(self.base_path, x))
                ),
                entries,
            )
        )
    ...
    def render(
        ...
        if self.manifest:
            ...
            if ncx_manifest_entry is not None and toc is not None:
                if not os.path.isabs(ncx_manifest_entry):
                    ncx_manifest_entry = os.path.join(
                        self.base_path, ncx_manifest_entry
                    )
        ...
        a = metadata.append
        role = {}
        a(DC_ELEM("title", self.title if self.title else "Unknown", opf_attrs=role))
        ...
        raw = etree.tostring(
            root, pretty_print=True, xml_declaration=True, encoding=encoding
        )
...
def metadata_to_opf(mi, as_string=True, default_lang=None):
    ...
    if not mi.book_producer:
        mi.book_producer = (
            __appname__ + " (%s) " % __version__ + "[https://calibre-ebook.com]"
        )

    if not mi.languages:
        lang = (
            get_lang().replace("_", "-").partition("-")[0]
    ...
    def factory(
        tag, text=None, sort=None, role=None, scheme=None, name=None, content=None
    ):
        attrib = {}
        if sort:
        ...
        try:
            ...
            elem = metadata.makeelement(
                tag, attrib={k: clean_xml_chars(v) for k, v in attrib.items()}
            )
        elem.tail = "\n" + (" " * 8)
        if text:
            try:
