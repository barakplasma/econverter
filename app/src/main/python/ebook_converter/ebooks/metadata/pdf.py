...
def read_info(outputdir, get_cover):
    ...
    try:
        ...
        print(f"pdfinfo failed to read XML metadata with return code: {e.returncode}")
    else:
    ...
    if get_cover:
        try:
            subprocess.check_call(
                [pdftoppm, "-singlefile", "-jpeg", "-cropbox", source_file, cover_file]
            )
        except subprocess.CalledProcessError as e:
...
def page_images(
    pdfpath, outputdir=".", first=1, last=1, image_format="jpeg", prefix="page-images"
):
    pdftoppm = "pdftoppm"
