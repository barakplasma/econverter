...
try:
    ...
    print(
        f"Author split regexp: {tweaks['authors_split_regex']}, is invalid, "
        f"using default"
    )
...
def author_to_author_sort(author, method=None):
    ...
    prefixes = {uenc.force_unicode(y).lower() for y in tweaks["author_name_prefixes"]}
    ...
    suffixes = {uenc.force_unicode(y).lower() for y in tweaks["author_name_suffixes"]}
...
_ignore_starts = "'\"" + "".join(
    chr(x) for x in list(range(0x2018, 0x201E)) + [0x2032, 0x2033]
)
