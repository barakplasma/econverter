...
def serialize_xmp_packet(root, encoding="utf-8"):
    ...
    raw_bytes = etree.tostring(
        root, encoding=encoding, pretty_print=True, with_tail=True, method="xml"
    )
    return (
...
def read_xmp_identifers(parent):
    ...
    for li in XPath("./rdf:Bag/rdf:li")(parent):
        ...
        is_resource = is_resource or (
            len(li) == 1 and li[0].tag == expand("rdf:Description")
        )
        if not is_resource:
...
def metadata_from_xmp_packet(raw_bytes):
    ...
    if title:
        if title.startswith(r"\376\377"):
            ...
            raise ValueError(
                "Corrupted XMP metadata packet detected, "
    ...
    try:
        pubdate = parse_date(
            first_sequence("//dc:date", root) or first_simple("//xmp:CreateDate", root),
            assume_utc=False,
        )
    except Exception:
        pass
...
def consolidate_metadata(info_mi, info):
    ...
    if prefer_info:
        ...
        # of the rest
        info_mi.authors = info_authors if xmp_mi.is_null("authors") else xmp_mi.authors
...
def metadata_to_xmp_packet(mi):
    ...
    if not mi.is_null("pubdate"):
        ...
        create_sequence_property(dc, "dc:date", [isoformat(mi.pubdate, as_utc=False)])
    if not mi.is_null("languages"):
        langs = list(
            filter(
                None,
                map(
                    lambda x: lang_as_iso639_1(x) or canonicalize_lang(x), mi.languages
                ),
            )
        )
        if langs:
    ...
    calibre = rdf.makeelement(
        expand("rdf:Description"), nsmap=nsmap("calibre", "calibreSI", "calibreCC")
    )
    ...
    if not mi.is_null("timestamp"):
        create_simple_property(
            calibre, "calibre:timestamp", isoformat(mi.timestamp, as_utc=False)
        )
    for x in ("author_link_map", "user_categories"):
