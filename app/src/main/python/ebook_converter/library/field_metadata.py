...
class FieldMetadata(object):
    ...
    def __init__(self):
        ...
        for k, v in self._field_metadata:
            if v["kind"] == "field" and v["datatype"] not in self.VALID_DATA_TYPES:
                raise ValueError(
                    "Unknown datatype %s for field %s" % (v["datatype"], k)
                )
            self._tb_cats[k] = v
    ...
    def standard_field_keys(self):
        return [
            k
            for k in self._tb_cats.keys()
            if self._tb_cats[k]["kind"] == "field" and not self._tb_cats[k]["is_custom"]
        ]
    ...
    def custom_field_keys(self, include_composites=True):
        ...
        for k in self._tb_cats.keys():
            ...
            if (
                fm["kind"] == "field"
                and fm["is_custom"]
                and (fm["datatype"] != "composite" or include_composites)
            ):
                res.append(k)
        return res
    ...
    def _remove_grouped_search_terms(self):
        to_remove = [
            v
            for v in self._search_term_map
...
def fm_as_dict(self):
    return {
        ...
        "user_categories": {
            k: v for k, v in self._tb_cats.items() if v["kind"] == "user"
        },
        "search_categories": {
            k: v for k, v in self._tb_cats.items() if v["kind"] == "search"
        },
    }
