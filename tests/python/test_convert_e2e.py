...
def test_bomless_utf16le_markdown_to_azw3(tmp_path):
    _input_path, output_path, _result = run_convert(
        tmp_path,
        "kindle-usb-e2e.md",
        MARKDOWN_FIXTURE.encode("utf-16-le"),
        "kindle-usb-e2e.azw3",
    )
...
def test_bomless_utf16le_plain_text_to_epub(tmp_path):
    source = (
        f"Plain text sanitizer\x00 keeps readable text — {HEBREW}.\n\nSecond paragraph."
    )
    input_path, output_path, _result = run_convert(
        tmp_path, "plain-control.txt", source.encode("utf-16-le"), "plain-control.epub"
    )
...
def test_markdown_relative_image_is_embedded(tmp_path):
    ...
    assert "body text after the image" in visible_text(epub_document_text(entries))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "unclosed-tags",
            "Text with <table><tr><td>unclosed table\n\nafter the table\n",
            "unclosed table",
        ),
        (
            "rtl",
        ...
        (
            "control-soup",
            "A\x00B\x01C\x1fD normal text continues here\n\nnext ¶\n",
